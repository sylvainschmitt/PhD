...
# gap <- rasterToPolygons(gap)
gap <- shapefile("./environment_save/gap/gap.shp")
gap$area <- area(gap)
# save(gap, file = "./environment_save/gap.Rdata")
# load("./environment_save/gap.Rdata")
# gap <- subest(gap, area > 4)
# save(gap, file = "./environment_save/gap2.Rdata")
load("./environment_save/gap2.Rdata")
...
```{r curvature map}
curvature <- raster("./environment_save/env/curvature.tiff")
dem <- raster(file.path(path, "topography", "MNT_ParacouAvril2009_5m.tif")) # for CRS
...
trees <- trees %>% # tree selection
  ...
  select(idArbre, n_parcelle, n_carre, n_arbre, espece,
         Xutm, Yutm, dbh_c, growth)
treesXY <- trees
...
trees %>% 
  filter(espece != "Indet.") %>% 
  ggplot(aes(wetness, growth/dbh_c)) +
  geom_point() +
  facet_wrap(~espece)
# growth <- as_data(trees$growth)
# dbh <- as_data(trees$dbh_c)
# wet <- as_data(trees$wetness)
# mu <- gamma(10^-2,10^-2)
# alpha <- gamma(10^-2,10^-2)
# beta <- gamma(10^-2,10^-2)
# sigma <- gamma(10^-2,10^-2, truncation = c(0,10))
# distribution(growth) <- normal(mu + alpha*dbh^2 + beta*wet, sigma)

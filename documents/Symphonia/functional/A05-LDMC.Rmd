...
$$LDMC_i \sim \mathcal{N} (\sum _{j<20m} DBH_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
```{stan Mcomp, output.var="Mcomp", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LDMC[n] ~ normal(NCI[n], sigma/NCI[n]) ;
  }
  generated quantities {
    real LDMCpred[N] ;
    for(n in 1:N)
...
fit <- sampling(Mcomp, chains = 2,
                data = list(N = nrow(arrange(dataP16, idTree)),
                            ...
                            LDMC = arrange(dataP16, idTree)$LDMC,
...
```{r LDMCAll}
data %>% 
  dplyr::select(Bark, LDMC, DBH, TWI) %>%
  mutate(DBH0 = DBH) %>% 
...
$$LDMC \sim \mathcal{N} (\frac{\alpha_{DBH} * dbh}{dbh + \beta_{DBH}} +  \beta_{TWI}*TWI, \sigma) $$
...
```{stan Mall, output.var="Mall", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LDMC[n] ~ normal((alpha_DBH*DBH[n])/(DBH[n] + beta_DBH) + beta_TWI*TWI[n], sigma) ;
  }
  generated quantities {
    real LDMCpred[N] ;
    for(n in 1:N)
      LDMCpred[n] = (alpha_DBH*DBH[n])/(DBH[n] + beta_DBH) + beta_TWI*TWI[n] ;
  }
```
...
#                             DBH = data$DBH,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelLDMCall.Rdata")
load("./functional_save/ModelLDMCall.Rdata")

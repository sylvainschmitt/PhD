...
Competition %>% 
  ...
  ggplot(aes(NBI, LA, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$LA_i \sim \mathcal{N} (\sum _{j<20m} S_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
```{r LAAll}
data %>% 
  dplyr::select(Bark, LA, DBH, TWI) %>%
  mutate(DBH0 = DBH) %>% 
  reshape2::melt(id.vars = c("Bark", "LA", "DBH0")) %>% 
  ggplot(aes(value, LA, size = DBH0, aes = DBH0,
             col = Bark, group = NA)) +
  geom_point() +
  facet_wrap(~ variable) +
  geom_smooth()
...
$$LA \sim \mathcal{N} (\frac{dbh + \beta_{DBH}}{\alpha_{DBH} * dbh} +  \beta_{TWI}*TWI, \sigma) $$
...
```{stan Mall, output.var="Mall", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LA[n] ~ normal((DBH[n] + beta_DBH)/(alpha_DBH*DBH[n]) + beta_TWI*TWI[n], sigma) ;
  }
  generated quantities {
    real LApred[N] ;
    for(n in 1:N)
      LApred[n] = (DBH[n] + beta_DBH)/(alpha_DBH*DBH[n]) + beta_TWI*TWI[n] ;
  }
```
...
#                             DBH = data$DBH,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelLAall.Rdata")
load("./functional_save/ModelLAall.Rdata")
...
cbind(
  ...
  geom_point(aes(y = LA, size = DBH0, aes = DBH0, col = Bark)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +

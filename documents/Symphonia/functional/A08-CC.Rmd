...
Competition %>% 
  ...
  ggplot(aes(NBI, CC, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$CC_i \sim \mathcal{N} (\sum _{j<20m} S_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
```{r CCAll}
data %>% 
  dplyr::select(Bark, CC, DBH, TWI) %>%
  mutate(DBH0 = DBH) %>% 
  reshape2::melt(id.vars = c("Bark", "CC", "DBH0")) %>% 
  ggplot(aes(value, CC, size = DBH0, aes = DBH0,
             col = Bark, group = NA)) +
  geom_point() +
  facet_wrap(~ variable) +
  geom_smooth()
...
$$CC \sim \mathcal{N} (\frac{\alpha_{DBH} * dbh}{dbh + \beta_{DBH}} +  \beta_{TWI}*TWI, \sigma) $$
...
```{stan Mall, output.var="Mall", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      CC[n] ~ normal((alpha_DBH*DBH[n])/(DBH[n] + beta_DBH) + beta_TWI*TWI[n], sigma) ;
  }
  generated quantities {
    real CCpred[N] ;
    for(n in 1:N)
      CCpred[n] = (alpha_DBH*DBH[n])/(DBH[n] + beta_DBH) + beta_TWI*TWI[n] ;
  }
```
...
#                             DBH = data$DBH,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelCCall.Rdata")
load("./functional_save/ModelCCall.Rdata")
...
cbind(
  ...
  geom_point(aes(y = CC, size = DBH0, aes = DBH0, col = Bark)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +

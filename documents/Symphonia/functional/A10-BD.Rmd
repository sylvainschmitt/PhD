...
Competition %>% 
  ...
  ggplot(aes(NBI, brBD, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$BD_i \sim \mathcal{N} (beta_{competition}*\sum _{j<20m} DBH_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
```{r BDAll}
data %>% 
  dplyr::select(Bark, brBD, DBH, TWI) %>%
  mutate(DBH0 = DBH) %>% 
...
$$BD \sim \mathcal{N} (\frac{dbh + BD_{min}}{\alpha * dbh} +  \beta_{TWI}*TWI), \sigma) $$
...
```{stan Mall, output.var="Mall", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      BD[n] ~ normal((DBH[n] + BD_min)/(alpha*DBH[n]) + beta_TWI*TWI[n], sigma) ;
  }
  generated quantities {
    real BDpred[N] ;
    for(n in 1:N)
      BDpred[n] = (DBH[n] + BD_min)/(alpha*DBH[n]) + beta_TWI*TWI[n] ;
  }
```
...
#                             DBH = data$DBH,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelbrBDall.Rdata")
load("./functional_save/ModelbrBDall.Rdata")
pars <- c("alpha", "BD_min", "beta_TWI", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
  kable(caption = "Summary table of the model dbh only")
```
...
cbind(
  ...
  geom_point(aes(y = brBD, size = DBH0, aes = DBH0, col = Bark)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +

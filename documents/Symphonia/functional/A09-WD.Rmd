...
Competition %>% 
  ...
  ggplot(aes(NBI, brWD, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$WD_i \sim \mathcal{N} (beta_{competition}*\sum _{j<20m} DBH_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
$$WD_i \sim \mathcal{N} (\sum _{j<20m} S_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
```{r WDAll}
data %>% 
  dplyr::select(Bark, brWD, DBH, TWI) %>%
  mutate(DBH0 = DBH) %>% 
  reshape2::melt(id.vars = c("Bark", "brWD", "DBH0")) %>% 
  ggplot(aes(value, brWD, size = DBH0, aes = DBH0,
             col = Bark, group = NA)) +
  geom_point() +
  facet_wrap(~ variable) +
  geom_smooth()
...
$$WD \sim \mathcal{N} (\frac{dbh + WD_{min}}{\alpha * dbh} +  \beta_{TWI}*TWI), \sigma) $$
...
```{stan Mall, output.var="Mall", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      WD[n] ~ normal((DBH[n] + WD_min)/(alpha*DBH[n]) + beta_TWI*TWI[n], sigma) ;
  }
  generated quantities {
    real WDpred[N] ;
    for(n in 1:N)
      WDpred[n] = (DBH[n] + WD_min)/(alpha*DBH[n]) + beta_TWI*TWI[n] ;
  }
```
...
#                             DBH = data$DBH,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelbrWDall.Rdata")
load("./functional_save/ModelbrWDall.Rdata")
pars <- c("alpha", "WD_min", "beta_TWI", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
  kable(caption = "Summary table of the model dbh only")
```
...
cbind(
  ...
  geom_point(aes(y = brWD, size = DBH0, aes = DBH0, col = Bark)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +

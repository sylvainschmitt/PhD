...
```{r LTCompetition}
Competition %>% 
  filter(idTreei != idTreej) %>% 
  group_by(idTreei) %>% 
  summarise(NBI = sum(DBH/1000*exp(-dij))) %>% 
  left_join(data, by = c("idTreei" = "idTree")) %>% 
  ggplot(aes(NBI, LT, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$LT_i \sim \mathcal{N} (\sum _{j<20m} S_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
$$LT \sim \mathcal{N} (\frac{\alpha_{DBH} * dbh}{dbh + \beta_{DBH}} +  \beta_{TWI}*TWI, \sigma) $$
...
```{stan Mall, output.var="Mall", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LT[n] ~ normal((alpha_DBH*DBH[n])/(DBH[n] + beta_DBH) + beta_TWI*TWI[n], sigma) ;
  }
  generated quantities {
    real LTpred[N] ;
    for(n in 1:N)
      LTpred[n] = (alpha_DBH*DBH[n])/(DBH[n] + beta_DBH) + beta_TWI*TWI[n] ;
  }
```
...
#                             DBH = data$DBH,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelLTall.Rdata")
load("./functional_save/ModelLTall.Rdata")

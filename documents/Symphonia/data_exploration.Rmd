...
## Carré
if(!all(ind$Carré %in% 1:4))
  warning("Lines ", paste(which(!(ind$Carré %in% 1:16)), collapse = ', '),
...
## Bois
if(!all(ind$Bois %in% 0:1))
  warning("Lines ", paste(which(!(ind$Bois %in% 0:1)), collapse = ', '),
...
if(!all(SPAD$Parcelle %in% 1:16))
  warning("Lines ", paste(which(!(SPAD$Parcelle %in% 1:16)), collapse = ', '),
          " have a wrong Parcelle number.\n")
## Carré
if(!all(SPAD$Carré %in% 1:4))
  warning("Lines ", paste(which(!(SPAD$Carré %in% 1:16)), collapse = ', '),
          " have a wrong Carré number.\n")
## Numéro
...
if(!all(fresh$Parcelle %in% 1:16))
  warning("Lines ", paste(which(!(fresh$Parcelle %in% 1:16)), collapse = ', '),
          " have a wrong Parcelle number.\n")
## Carré
if(!all(fresh$Carré %in% 1:4))
  warning("Lines ", paste(which(!(fresh$Carré %in% 1:16)), collapse = ', '),
          " have a wrong Carré number.\n")
## Numéro
...
if(!all(fresh$Feuille %in% 1:4000))
  warning("Lines ", paste(which(!(fresh$Feuille %in% 1:4000)), collapse = ', '),
          " have a wrong Feuille number.\n")
## LT
test <- function(x) ((x > 100 & x < 600) | is.na(x))
...
## Poids frais
test <- function(x) ((x > 0.1 & x < 3) | is.na(x))

...
# save(symphonia, file = "./Competition_save/symphoniaBA.Rdata")
load("./Competition_save/symphoniaBA.Rdata")
symphonia <- symphonia %>%
...
symphonia <- symphonia %>% 
  mutate(BA10 = BA10/10) %>% 
  mutate(BA20 = BA20/10) %>% 
  filter(!(CensusYear %in% c(2008, 2010, 2012, 2014, 2016))) # partial inventory
symphonia <- symphonia %>% 
...
symphonia %>% 
  select(BA10, BA20, Species, Treatment) %>% 
  reshape2::melt(id.var = c("Species", "Treatment")) %>% 
...
lot(aes(CensusYear, BA)) + geom_point() + geom_line() + facet_wrap(~Plot)
...
symphonia %>% 
  filter(CensusYear > 1995) %>% 
  select(Species, idTree, BA10, BA20, CensusYear, Treatment) %>% 
  reshape2::melt(id.vars = c("Species", "idTree", "CensusYear", "Treatment"),
                 variable.name = "BAdist", value.name = "BA") %>% 
  ggplot(aes(CensusYear, BA, col = Species)) +
  geom_point() + 
  geom_line(aes(group = idTree)) +
  facet_grid(Treatment ~ BAdist, scale = "free") +
...
symphonia %>% 
  select(Species, idTree, BA10, BA20, dbh, Treatment) %>% 
  reshape2::melt(id.vars = c("Species", "idTree", "dbh", "Treatment"),
                 variable.name = "BAdist", value.name = "BA") %>% 

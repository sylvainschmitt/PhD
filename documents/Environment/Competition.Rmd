...
symphonia %>% 
  ...
  reshape2::melt(id.vars = c("Species", "idTree", "dbh"),
                 variable.name = "BAdist", value.name = "BA") %>% 
  ggplot(aes(BA, dbh, col = Species)) +
  geom_point() + 
  facet_wrap(~ BAdist, scale = "free", nrow = 2) +
  scale_color_discrete("Symphonia") +
  xlab("Local basal area (BA in m2/ha)") +
  ylab("Diameter at breast height (dbh in cm)")
```
...
symphonia %>% 
  select(Species, idTree, BA10, BA20, agr) %>% 
  reshape2::melt(id.vars = c("Species", "idTree", "agr"),
                 variable.name = "BAdist", value.name = "BA") %>% 
  ggplot(aes(BA, agr, col = Species)) +
  geom_point() + 
  facet_wrap(~ BAdist, scale = "free", nrow = 2) +
  scale_color_discrete("Symphonia") +
  xlab("Local basal area (BA in m2/ha)") +

```{r Ch3Setup, message=FALSE, warning=FALSE, include=FALSE}
...
load("rdata/Ch3Msc.Rdata")
```

# Chapter 3: Topography drives microgeographic adaptations of closely-related species in two tropical tree species complexes {#Ch3}
...
Niklas Tysklind$^2$,
Bruno Hérault$^{3,4,5}$ [![](images/orcid.png)](https://orcid.org/0000-0002-6950-7286), 
...
were sampled in 2017 and 2018 during the dry season (i.e. from September to December) in Paracou (Fig. \@ref(fig:Ch3maps)).
...
```{r Ch3maps, fig.cap="Map of sampled individuals for species complexes *Symphonia* and *Eschweilera* clade *Parvifolia*. Colours represent species determined based on the genetic structure of capture datasets, which was not necessarily in agreement with original botanical identification. Grey level represents hill shading to visualize topography.", fig.width=6, fig.height=5}
...
We designed in *silico* two sets of 20,000 80-mer probes for sequence capture, one for *Symphonia globulifera*, and a second one compatible with both *Eschweilera sagotiana* and *Eschweilera coriacea* (Fig. \@ref(fig:A4symcapture), Fig. \@ref(fig:A4parvicapture)). 
...
Details on the design of the probes set for *Symphonia* are given in Method S[1](#A4methodS1). 
...
Details on the design of the probes set for *Eschweilera* are given in Method S[2](#A4methodS2). 
...
We automatically tested numerous values of missing data and outgroup filtering, and kept strict filters with population structure congruent with botanical identification but keeping a maximum of individuals (Fig. \@ref(fig:A4libmiss), Fig. \@ref(fig:A4snpmiss)).
...
Outgroups were automatically filtered out by clustering individuals in two groups using K-means in a genomic principal component analysis (Fig. \@ref(fig:A4outgroup)).
...
We used two relatively-independent topographic variables, the topographic wetness index (TWI) and the relative elevation (RE, orthogonality shown in Fig. \@ref(fig:Ch3soilpca)), as proxies of the distribution of soil water and nutrients availability in Paracou. 
...
A Bayesian method was used to infer parameters using `stan` language [@Carpenter2017, code available in Model S[1](#A4model)] and `rstan` package [@Team2018] in the R environment [@RCoreTeam2020] using the No-U-Turn Sampler alogirthm [NUTS, @Hoffman2014], which performs better for estimating genetic parameters and breeding values [@Nishio2019].

## Results

*Symphonia globulifera* was previously recognized as composed of two morphotypes in French Guiana,
but cross validation of admixture analyses for different levels of clustering advocated for K = 3 different gene pools (Fig. \@ref(fig:Ch3symphospecies)a, Fig. \@ref(fig:A4admixtureSympoCV), Fig. \@ref(fig:A4admixtureSympo)).
...
These two gene pools correspond to two locally defined sub-morphotypes herafter called *S. globulifera type Paracou* and *S. globulifera type Régina* (Fig. \@ref(fig:Ch3symphospecies)b). 
*Symphonia sp.1* grows preferentially in drier plateaux and slopes (Schmitt et al., submitted) and has a light grey thin and smooth bark associated with smaller leaves, flowers, and fruits (Fig. \@ref(fig:Ch3symphospecies)b). 
*Symphonia globulifera type Paracou* grows in wet habitats of bottomlands, but drier than those of *S. globulifera type Régina*, and has a dark bark that is intermediate between that of the other morphotypes with respect to thickness and rugosity (Fig. \@ref(fig:Ch3symphospecies)b). 
*Symphonia globulifera type Régina* grows in the wettest bottomlands of Paracou and has a thick and lashed bark (Fig. \@ref(fig:Ch3symphospecies)b).
...
whereas no admixed individuals were found between *S. globulifera type Régina* and *S. globulifera type Paracou* (confirmed with `introgress` R package, Fig. \@ref(fig:A4introgress)). 
...
cowplot::plot_grid(
  ...
  scale_fill_manual("", values = c("#CD2626", "#1E90FF", "#0000EE"),
                    labels = c("S. sp.1", "S. globulifera Paracou", "S. globulifera Régina")) +
  ...
  cowplot::ggdraw() + 
    cowplot::draw_image("../genomics/topography/figures/symphoniamorphotypes.png", scale = 0.9),
  nrow = 2, labels = c("(a)", "(b)")
)
```

Genomic scans for *Symphonia* revealed high-differentiation outliers among gene pools (5.7% of SNPs, Fig. \@ref(fig:A4bayescanOutliers)),
...
corresponding to three distinct morphotypes (Fig. \@ref(fig:Ch3symphospecies)). 
...
The gene pools corresponded to botanical species *E. coriacea*, *E. sagotiana*, and *E. decolorans* (65-80% of match, Fig. \@ref(fig:Ch3parvispecies), Tab. \@ref(tab:A4kmeansConfusion)).
Symphonia and Eschweilera gene pools showed intermediate genetic differentiation, $F_{ST}=0.15$ and $F_{ST}=0.35$, respectively.

```{r Ch3parvispecies, fig.cap="Principal component analysis (PCA) of single nucleotide polymorphisms (SNP) from *Eschweilera* clade *Parvifolia.* The colours represent the clusters detected with Kmeans for the best K, K = 3.", fig.height=5, fig.width=5}
...
We confirmed topographic wetness index (TWI) and relative elevation (RE, Fig. \@ref(fig:Ch3SpDistVar)) as descriptors of microenvironmental variation along the topographic gradient exploited by the two species complexes in Paracou.
Topographic wetness index (TWI) was related to soil water distribution, opposing water tables deeper than 100 cm to those between the surface and 100 cm deep, which corresponded to hydromorphic soils with the most pronounced waterlogging (variable waterlog5, Fig. \@ref(fig:Ch3soilpca)).
Relative elevation (RE) was related to the distribution of nutrients, opposing hilltop soils with the lowest degree of waterlogging (waterlog1) and increased soil organic compounds (soil carbon, nitrogen, organic matter, exchangeable cation charge) to those further down the slopes with intermediate levels of waterlogging and higher soil fertility (higher availability of phosphorus) (Fig. \@ref(fig:Ch3soilpca)).

```{r Ch3soilpca, fig.cap="Principal component analysis (PCA) of topographic wetness index (TWI), relative elevation (RE) with edaphic and hydrological properties. Edaphic and hydrological properties include organic matter (OM), carbon (C), nitrogen (N), exchangeable cation charge (C) [@Soucemarianadin2004; @Roelens2007], water table depth [WTD in cm, @Gourlet-Fleury2004], hydromorphy and waterlogging [waterlog in 5 classes, @Cantet2004]). Data were obtained through interpolation of soil sample data on TWI and RE from LiDAR data."}
read_tsv("../genomics/topography/figures/soil.tsv") %>%
  na.omit() %>% 
...
The distribution of *Symphonia* and *Eschweilera* species along the topographic gradient suggested that both topographic wetness index (TWI) and relative elevation (RE) drove individual survival across gene pools ($0.06<R_m^2<0.39$, Fig. \@ref(fig:Ch3SpDistVar), Tab. \@ref(tab:Ch3fits)).
The spatial distribution of *Symphonia* species was driven by both the topographic wetness index (TWI, Fig. \@ref(fig:Ch3SpDistVar)**c**, Tab. \@ref(tab:Ch3fits)), especially between *S. globulifera type Régina* and *S. globulifera type Paracou*, and the relative elevation (RE, Fig. \@ref(fig:Ch3SpDistVar)**d**, Tab. \@ref(tab:Ch3fits)), especially between *S. sp.1* and the two other species.
The spatial distribution of *Eschweilera*  species was more driven by relative elevation (RE, Fig. \@ref(fig:Ch3SpDistVar)**b**, Tab. \@ref(tab:Ch3fits)) than by topographic wetness index.
...
lapply(fitEnv, broom::tidyMCMC, c("mu", "Vp", "Vg", "Vr"), droppars = NULL, rhat = F) %>% 
  ...
  mutate(population = recode(population, 
                             ...
                             "Symphonia2" = "S. globulifera Régina", 
                             "Symphonia3" = "S. sp.1",
  ...
  mutate(parameter = recode(parameter, 
                            ...
                            "Vg" = "$V_{genotype}$",
                            "Vr" = "$V_{residuals}$")) %>% 
  kable(caption = "Parameters and associated standard deviation of the animal model for *Symphonia* and *Eschweilera* with topographic wetness index (TWI) and relative elevation (RE). Parameters include species mean ($\\mu_s$), variance ($V_{species}$), genotype variance ($V_{genotype}$), and residuals variance ($V_{residuals}$). Parameters have been infered with bayesian modelling of the animal model (Model S1) independently for the two species complexes and the two topographic variables.",
        escape = F, format = "pandoc", booktabs = T, digits = 3,
        col.names = c("Species complex", "Variable", "Parameter",  
...
with a ca. five-fold larger inter- than intra-specific component (Fig. \@ref(fig:Ch3SpDistVar)).
...
characterized by low levels of waterlogging and soils rich in organic compounds [Fig. \@ref(fig:Ch3SpDistVar); @Allie2015].
...
whereas the  topographic wetness index best captured genetic variation among genotypes (78%, Fig. \@ref(fig:Ch3SpDistVar)).

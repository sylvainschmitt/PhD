...
if(dim(distinct(select(LAcor,Plot,SubPlot,TreeFieldNum)))[1]!=dim(individuals)[1])
   warning(paste(dim(distinct(select(LAcor,Plot,SubPlot,TreeFieldNum)))[1],collapse=",")," individuals in LAcor wheareas ",paste(dim(individuals)[1],collapse=",")," in Individual")
```
...
LAfinal <- read_delim(file.path(path1,"Individuals_all.csv"),delim=",") %>% 
  group_by(Plot,SubPlot,TreeFieldNum) %>% 
  count() %>% 
  filter(n!=5) %>% 
  kable(format = "html", escape = F,
...
paracou %>% 
  ...
  ungroup() %>% 
  left_join(dry, by = c("IdGenetic", "Plot", "SubPlot", "TreeFieldNum", "Leaf"),
  suffix = c("", ".dry")) %>%
  mutate(LAexclude = NA, LAinclude = NA) %>% 
...
```{r AllTraits, eval=F}
measures <- googlesheets::gs_title("Measures_Eschweilera") %>% 

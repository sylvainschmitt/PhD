...
* __P14-1-760__ all leaves, is a natural outlier identified in the PCA due to an anormally high SLA due to the fact that the sample comse from a tree rejection grown on a beheaded tree. Was removed from the datatable for further outliers detection
...
data %>% 
  ...
  DT::datatable(options = list(
    ...
    initComplete = htmlwidgets::JS(
      ...
      "}")))
```
On the PCA we spotted other indviduals that could be considered as outliers:
1. __13-2-881__ seemed to be offset on the PCA, but after checking value all traits were in a normal range except for the CC. Leaves sampled seemed to be young ones and consequently were light, what can explain a low SPAD
1. __16-3-260__ seemed to be offset on the PCA, what could be due to abnormal values of LDMC and SLA. We checked value of LeafDry, and they me be should be done again 
1. __16-7-185-5__ have a particularly high SLA regarding other leaves of the same individual. LA values seemed to be not well attributed regarding leaves scan. LA values weren't well attributed. LA data table has been updated for this purpose

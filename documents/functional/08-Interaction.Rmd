...
```{r setup_interactionInterIntra, include=FALSE}
...
```{r data_interactionInterIntra, eval=T}
...
```{r species_interactionInterIntra}
species <- Individuals %>% 
  dplyr::select(SpeciesLong, Genus, Species, SpeciesNum) %>% 
  unique()
```

```{r predicitons_interactionInterIntra}
traits <- c("LMA", "LDMC", "LT", "invLA", "CC")
complexes <- c("Symphonia", "Eschweilera")
models <- sapply(complexes, function(complex) paste0(complex, "-", traits))
...
model {
  ...
  sigma_a ~ normal(0, 1) ;
  sigma_t ~ normal(0, 1) ;
  Trait ~ normal((alpha_s[species] + betaTWI*TWIs[species] +  gammaTWI[species] .* TWIis) .* (DBH ./ (betaDBH[species] + DBH)), sigma_a) ; // Likelihood
}
```
...
mcmc_trace(as.array(fits$`Symphonia-LT`, pars = c("alpha",  "sigma_a", "betaDBH", 
                                                  "betaTWI", "gammaTWI", 
                                                  "sigma_t", "lp__")),
...
mcmc_pairs(as.array(fits$`Symphonia-LT`, pars = c("sigma_a", "sigma_t")))
```

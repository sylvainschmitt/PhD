...
$\{beta_{DBH}}_{90}$ was computed as the value of DBH - when DBH not reduced nor centered - for which the trait account for 90% of its maximum value, as far as ${\beta_{DBH}}_{c}$ ranged in values Result are presented in table x.
...
$\sigma^2_{TWI}$ took values between 0.05 and 0.1, which represented for significant $\beta_{TWI}$ always more than 100% of their value.
$\sigma^2_{intercept}$, $\sigma^2_{DBH}$ and $\sigma^2_{TWI}$ ranged in the order of $10^{-2}$. Two major trends could be highlighted. LDMC and CC had $\sigma^2$, $\sigma^2_{intercept}$, $\sigma^2_{DBH}$ and $\sigma^2_{comp}$ closer to zero than LT and invSLA. $\sigma^2_{TWI}$ ranged in values quite similar for all traits although LT and invSLA had values slightly lower.
Notice also that for all $\sigma^2_{species}$, the confidence interval got larger with higher estimate values, yet $\sigma^2$ had values of the highest order of magnitude but had a narrow confidence interval.

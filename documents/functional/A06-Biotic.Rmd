...
mdata <- lapply(traits, function(trait){
  ...
  Competition_trait <- filter(Competition_trait, dij < dmax)
...
$$e^{-alpha.\delta_{1\%}}=0.01 \iff \delta_{1\%} = -\frac{2.log(10)}{\alpha}$$

```{r, eval=F, fig.height=12, fig.width=12}
ggplot(data.frame(x = 0), aes(x)) +
  stat_function(fun = function(x) -log(0.01)/x) +
  xlim(0,1)
data.frame(alpha = seq(0, 1, by = 0.1), 
           `delta-1pct` = sapply(seq(0, 1, by = 0.1), function(x) -log(0.01)/x)) %>% 
  kable(col.names = c("$\\alpha$", "\\delta_{1\\%}"))
cowplot::plot_grid(plotlist = lapply(seq(0.1, 1, by = 0.1), function(alpha)
  Competition %>% 
    mutate(NCIj20 = DBHj^2*exp(-alpha*dij)) %>% 
    mutate(NCIj10 = ifelse(dij < 10, NCIj20, 0)) %>% 
    group_by(idTree) %>% 
    summarise(NCI20 = sum(NCIj20), NCI10 = sum(NCIj10)) %>% 
    ggplot(aes(NCI20, NCI10)) +
    geom_point() +
    geom_abline(slope = 1, intercept = 0)), labels = seq(0.1, 1, by = 0.1))
```
...
model {
  alphaNCI ~ lognormal(0, 10) ;
  sigmaComp ~ lognormal(0, 10) ;
...
> 100 observations ~100s for 20 m against 100 observations ~22s for 10m !

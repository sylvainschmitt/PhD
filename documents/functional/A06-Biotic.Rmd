...
```{r }
traits <- traits[!(traits %in% "invSLA")]
for(trait in traits){
  cat("### ", trait, " ### \n\n")
...
load("./functional_save/NCI_LDMC.Rdata")
pars <- c("alpha_c", "betaComp_c", "alphaNCI", "sigma")
...
mcmc_intervals(as.array(fit), regex_pars = pars)
```
...
cowplot::plot_grid(plotlist = lapply(traits, function(trait)
  ...
  labels = names(data), nrow = 2)
mcmc_intervals(as.array(fit), regex_pars = "Comp")
```

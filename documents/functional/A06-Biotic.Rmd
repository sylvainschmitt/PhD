...
mdata <- lapply(traits, function(trait){
   ...
   Competition_trait$idTree <- match(Competition_trait$idTree, 
data_trait$idTree)
   ...
   list(N = nrow(data_trait),
        ...
        speciesincomplex = 
unique(cbind(as.numeric(as.factor(data_trait$Species)), 
as.numeric(as.factor(data_trait$Genus))))[order(unique(cbind(as.numeric(as.factor(data_trait$Species)), 
as.numeric(as.factor(data_trait$Genus))))[,1]),2])})
names(mdata) <- traits
...
parameters {
  real alpha ;
  real beta_comp ;
  real<lower=0> alpha_NCI ;
  real<lower=0> sigma ;
}
transformed parameters {
  vector [N] NCI ;
  for(j in 1:J)
    NCI[individual[j]] = NCI[individual[j]] + DBH[j]^2*exp(-alpha_NCI*Deltaj[j]) ;
  }
model {
  Trait ~ normal(alpha + beta_comp*NCI, sigma) ; // Likelihood
} 

...
traits <- googlesheets::gs_title("Measures") %>% 
  ...
  mutate(LA = as.numeric(LA), LA = as.numeric(LA))
```
...
First model for LA variation in relation to environment.
...
```{r LADBH}
data %>% 
  ggplot(aes(DBH, LA, col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used an inverted Michealis Menten equation for DBH effect on LA.

$$LA \sim \mathcal{N} (\frac{dbh + \beta}{\alpha * dbh}, \sigma) $$
...
```{stan Mdbh, output.var="Mdbh", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LA[n] ~ normal((DBH[n] + beta)/(alpha*DBH[n]), sigma) ;
  }
  generated quantities {
    real LApred[N] ;
    for(n in 1:N)
      LApred[n] = (DBH[n] + beta)/(alpha*DBH[n]) ;
  }
```
...
#                             LA = data$LA,
#                             DBH = data$DBH))
# save(fit, file = "./functional_save/ModelLAdbh.Rdata")
load("./functional_save/ModelLAdbh.Rdata")
...
cbind(
  data,
  pred = apply(as.matrix(fit, pars = "LApred"), 2, mean),
  t(apply(as.matrix(fit, pars = "LApred"), 2, quantile, probs = c(0.05, 0.95)))
) %>% 
  ggplot(aes(x = DBH)) +
  geom_point(aes(y = LA)) +
...
We will used a linear equation for DBH effect on LA.

$$LA \sim \mathcal{N} (\alpha + \beta* dbh, \sigma) $$
...
```{stan MdbhLinear, output.var="MdbhLinear", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LA[n] ~ normal(alpha + beta*DBH[n], sigma) ;
  }
  generated quantities {
    real LApred[N] ;
    for(n in 1:N)
      LApred[n] = alpha + beta*DBH[n] ;
  }
```
...
#                             LA = data$LA,
#                             DBH = data$DBH))
# save(fit, file = "./functional_save/ModelLAdbhLinear.Rdata")
load("./functional_save/ModelLAdbhLinear.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
  kable(caption = "Summary table of the model dbh only")
```
...
```{r LATWI}
data %>% 
  ggplot(aes(TWI, LA, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for TWI effect on LA.

$$LA \sim \mathcal{N} (\alpha + \beta* TWI, \sigma) $$
...
```{stan MTWI, output.var="MTWI", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LA[n] ~ normal(alpha + beta*TWI[n], sigma) ;
  }
  generated quantities {
    real LApred[N] ;
    for(n in 1:N)
      LApred[n] = alpha + beta*TWI[n] ;
  }
```
...
#                             LA = data$LA,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelLATWILinear.Rdata")
load("./functional_save/ModelLATWILinear.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
...
Topographic wetness index as a significant negative effect on LA.
...
```{r LAElevation}
data %>% 
  ggplot(aes(DEM, LA, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for elevation effect on LA.

$$LA \sim \mathcal{N} (\alpha + \beta* Elevation, \sigma) $$
...
```{stan MElevation, output.var="MElevation", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LA[n] ~ normal(alpha + beta*Elevation[n], sigma) ;
  }
  generated quantities {
    real LApred[N] ;
    for(n in 1:N)
      LApred[n] = alpha + beta*Elevation[n] ;
  }
```
...
#                             LA = data$LA,
#                             Elevation = data$DEM))
# save(fit, file = "./functional_save/ModelLAElevation.Rdata")
load("./functional_save/ModelLAElevation.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
...
Competition %>% 
  ...
  ggplot(aes(NCI, LA, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$LA_i \sim \mathcal{N} (\sum _{j<20m} DBH_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
# save(fit, file = "./functional_save/ModelLACompetition.Rdata")
load("./functional_save/ModelLACompetition.Rdata")
pars <- c("alpha", "beta", "sigma")
...
cbind(
  ...
  ggplot(aes(x = NCI)) +
  geom_point(aes(y = LA)) +
  geom_text(aes(y = LA, label = idTreei)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed")
...
Gaps %>% 
  ...
  left_join(data) %>% 
  ggplot(aes(Idist, LA, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$LA_i \sim \mathcal{N} (\alpha + \beta*\sum _{j<20m} S_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
```{stan Mgaps, output.var="Mgaps", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LA[n] ~ normal(alpha + beta*Idist[n], sigma) ;
  }
  generated quantities {
    real LApred[N] ;
    for(n in 1:N)
      LApred[n] = alpha + beta*Idist[n] ;
  }
```
...
# save(fit, file = "./functional_save/ModelLAGaps.Rdata")
load("./functional_save/ModelLAGaps.Rdata")
...
cbind(
  ...
  geom_point(aes(y = LA, size = DBH)) +
  geom_text(aes(y = LA, label = idTree)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed")

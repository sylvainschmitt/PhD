...
traits <- googlesheets::gs_title("Measures") %>% 
  ...
  mutate(WD = as.numeric(WD), LDMC = as.numeric(LDMC))
```
...
First model for WD variation in relation to environment.
...
```{r WDDBH}
data %>% 
  ggplot(aes(DBH, brWD, col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for DBH effect on WD.

$$WD \sim \mathcal{N} (\alpha + \beta* dbh, \sigma) $$
...
```{stan MdbhLinear, output.var="MdbhLinear", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      WD[n] ~ normal(alpha + beta*DBH[n], sigma) ;
  }
  generated quantities {
    real WDpred[N] ;
    for(n in 1:N)
      WDpred[n] = alpha + beta*DBH[n] ;
  }
```
...
#                             WD = data$brWD,
#                             DBH = data$DBH))
# save(fit, file = "./functional_save/ModelbrWDdbhLinear.Rdata")
load("./functional_save/ModelbrWDdbhLinear.Rdata")
...
cbind(
  data,
  pred = apply(as.matrix(fit, pars = "WDpred"), 2, mean),
  t(apply(as.matrix(fit, pars = "WDpred"), 2, quantile, probs = c(0.05, 0.95)))
) %>% 
  ggplot(aes(x = DBH)) +
  geom_point(aes(y = brWD)) +
...
We will used an inverted Michealis Menten equation for DBH effect on WD.

$$WD \sim \mathcal{N} (\frac{dbh + \beta}{\alpha * dbh}, \sigma) $$
...
```{stan Mdbh, output.var="Mdbh", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      WD[n] ~ normal((DBH[n] + beta)/(alpha*DBH[n]), sigma) ;
  }
  generated quantities {
    real WDpred[N] ;
    for(n in 1:N)
      WDpred[n] = (DBH[n] + beta)/(alpha*DBH[n]) ;
  }
```
...
#                             WD = data$brWD,
#                             DBH = data$DBH))
# save(fit, file = "./functional_save/ModelbrWDdbh.Rdata")
load("./functional_save/ModelbrWDdbh.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
  kable(caption = "Summary table of the model dbh only")
```
...
```{r WDTWI}
data %>% 
  ggplot(aes(TWI, brWD, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for TWI effect on WD

$$WD \sim \mathcal{N} (\alpha + \beta* TWI, \sigma) $$
...
```{stan MTWI, output.var="MTWI", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      WD[n] ~ normal(alpha + beta*TWI[n], sigma) ;
  }
  generated quantities {
    real WDpred[N] ;
    for(n in 1:N)
      WDpred[n] = alpha + beta*TWI[n] ;
  }
```
...
#                             WD = data$brWD,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelbrWDTWI.Rdata")
load("./functional_save/ModelbrWDTWI.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
...
Topographic wetness index has a non significant negative effect on WD. 
...
```{r WDElevation}
data %>% 
  ggplot(aes(DEM, brWD, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for elevation effect on WD

$$WD \sim \mathcal{N} (\alpha + \beta* Elevation, \sigma) $$
...
```{stan MElevation, output.var="MElevation", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      WD[n] ~ normal(alpha + beta*Elevation[n], sigma) ;
  }
  generated quantities {
    real WDpred[N] ;
    for(n in 1:N)
      WDpred[n] = alpha + beta*Elevation[n] ;
  }
```
...
#                             WD = data$brWD,
#                             Elevation = data$DEM))
# save(fit, file = "./functional_save/ModelbrWDElevation.Rdata")
load("./functional_save/ModelbrWDElevation.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
...
Competition %>% 
  ...
  ggplot(aes(NCI, WD, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$WD_i \sim \mathcal{N} (\sum _{j<20m} DBH_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
# save(fit, file = "./functional_save/ModelWDCompetition.Rdata")
load("./functional_save/ModelWDCompetition.Rdata")
pars <- c("alpha", "beta", "sigma")
...
cbind(
  ...
  ggplot(aes(x = NCI)) +
  geom_point(aes(y = WD)) +
  geom_text(aes(y = WD, label = idTreei)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed")
...
Gaps %>% 
  ...
  left_join(data) %>% 
  ggplot(aes(Idist, WD, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$WD_i \sim \mathcal{N} (\alpha + \beta*\sum _{j<20m} S_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
```{stan Mgaps, output.var="Mgaps", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      WD[n] ~ normal(alpha + beta*Idist[n], sigma) ;
  }
  generated quantities {
    real WDpred[N] ;
    for(n in 1:N)
      WDpred[n] = alpha + beta*Idist[n] ;
  }
```
...
# save(fit, file = "./functional_save/ModelWDGaps.Rdata")
load("./functional_save/ModelWDGaps.Rdata")
...
cbind(
  ...
  geom_point(aes(y = WD, size = DBH)) +
  geom_text(aes(y = WD, label = idTree)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed")

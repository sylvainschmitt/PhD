...
traits <- googlesheets::gs_title("Measures") %>% 
  ...
  mutate(LDMC = as.numeric(LDMC), LDMC = as.numeric(LDMC))
```
...
First model for LDMC variation in relation to environment.
...
```{r LDMCDBH}
data %>% 
  ggplot(aes(DBH, LDMC, col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used an Michealis Menten equation for DBH effect on LDMC.

$$LDMC \sim \mathcal{N} (\frac{\alpha * dbh }{dbh + \beta}, \sigma) $$
...
```{stan Mdbh, output.var="Mdbh", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LDMC[n] ~ normal((alpha*DBH[n])/(DBH[n] + beta), sigma) ;
  }
  generated quantities {
    real LDMCpred[N] ;
    for(n in 1:N)
      LDMCpred[n] = (alpha*DBH[n])/(DBH[n] + beta) ;
  }
```
...
#                             LDMC = data$LDMC,
#                             DBH = data$DBH))
# save(fit, file = "./functional_save/ModelLDMCdbh.Rdata")
load("./functional_save/ModelLDMCdbh.Rdata")
...
cbind(
  data,
  pred = apply(as.matrix(fit, pars = "LDMCpred"), 2, mean),
  t(apply(as.matrix(fit, pars = "LDMCpred"), 2, quantile, probs = c(0.05, 0.95)))
) %>% 
  ggplot(aes(x = DBH)) +
  geom_point(aes(y = LDMC)) +
...
We will used a linear equation for DBH effect on LDMC.

$$LDMC \sim \mathcal{N} (\alpha + \beta* dbh, \sigma) $$
...
```{stan MdbhLinear, output.var="MdbhLinear", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LDMC[n] ~ normal(alpha + beta*DBH[n], sigma) ;
  }
  generated quantities {
    real LDMCpred[N] ;
    for(n in 1:N)
      LDMCpred[n] = alpha + beta*DBH[n] ;
  }
```
...
#                             LDMC = data$LDMC,
#                             DBH = data$DBH))
# save(fit, file = "./functional_save/ModelLDMCdbhLinear.Rdata")
load("./functional_save/ModelLDMCdbhLinear.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
  kable(caption = "Summary table of the model dbh only")
```
...
```{r LDMCTWI}
data %>% 
  ggplot(aes(TWI, LDMC, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for TWI effect on LDMC.

$$LDMC \sim \mathcal{N} (\alpha + \beta* TWI, \sigma) $$
...
```{stan MTWI, output.var="MTWI", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LDMC[n] ~ normal(alpha + beta*TWI[n], sigma) ;
  }
  generated quantities {
    real LDMCpred[N] ;
    for(n in 1:N)
      LDMCpred[n] = alpha + beta*TWI[n] ;
  }
```
...
#                             LDMC = data$LDMC,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelLDMCTWILinear.Rdata")
load("./functional_save/ModelLDMCTWILinear.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
...
Topographic wetness index as a significant negative effect on LDMC.
...
```{r LDMCElevation}
data %>% 
  ggplot(aes(DEM, LDMC, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for elevation effect on LDMC.

$$LDMC \sim \mathcal{N} (\alpha + \beta* Elevation, \sigma) $$
...
```{stan MElevation, output.var="MElevation", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LDMC[n] ~ normal(alpha + beta*Elevation[n], sigma) ;
  }
  generated quantities {
    real LDMCpred[N] ;
    for(n in 1:N)
      LDMCpred[n] = alpha + beta*Elevation[n] ;
  }
```
...
#                             LDMC = data$LDMC,
#                             Elevation = data$DEM))
# save(fit, file = "./functional_save/ModelLDMCElevation.Rdata")
load("./functional_save/ModelLDMCElevation.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
...
Competition %>% 
  ...
  ggplot(aes(NCI, LDMC, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$LDMC_i \sim \mathcal{N} (\alpha + \beta* \sum _{j<20m} DBH_j^{\beta_{NCI}} * e^{-\alpha_{NCI}*d_{i,j}} , \sigma) $$
...
```{stan Mcomp, output.var="Mcomp", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LDMC[n] ~ normal(alpha + beta*NCI[n], sigma) ;
  }
  generated quantities {
    real LDMCpred[N] ;
    for(n in 1:N)
      LDMCpred[n] = alpha + beta*NCI[n] ;
  }
```
...
# save(fit, file = "./functional_save/ModelLDMCCompetition.Rdata")
load("./functional_save/ModelLDMCCompetition.Rdata")
...
cbind(
  ...
  ggplot(aes(x = NCI)) +
  geom_point(aes(y = LDMC, size = DBH)) +
  geom_text(aes(y = LDMC, label = idTreei)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed")
...
Gaps %>% 
  ...
  full_join(data) %>% 
  ggplot(aes(Idist, LDMC, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$LDMC_i \sim \mathcal{N} (\sum _{j<20m} S_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
```{stan Mgaps, output.var="Mgaps", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LDMC[n] ~ normal(alpha + beta*Idist[n], sigma) ;
  }
  generated quantities {
    real LDMCpred[N] ;
    for(n in 1:N)
      LDMCpred[n] = alpha + beta*Idist[n] ;
  }
```
...
# save(fit, file = "./functional_save/ModelLDMCGaps.Rdata")
load("./functional_save/ModelLDMCGaps.Rdata")
...
cbind(
  ...
  geom_point(aes(y = LDMC, size = DBH)) +
  geom_text(aes(y = LDMC, label = idTree)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed")
...
data %>% 
  ...
  left_join(Gaps %>% 
              ...
              mutate(Distance = Distance/max(Distance)) %>% 
              summarise(Idist = sum(Area*exp(-Distance)))) %>% 
  dplyr::select(Bark, LDMC, DBH, TWI, NCI, Idist) %>%
  mutate(DBH0 = DBH) %>% 
  reshape2::melt(id.vars = c("Bark", "LDMC", "DBH0")) %>% 
  ggplot(aes(value, LDMC, size = DBH0, aes = DBH0,
             col = Bark, group = NA)) +
  geom_point() +
...
$$LDMC \sim \mathcal{N} (\frac{\alpha_{DBH} * dbh}{dbh + \beta_{DBH}} +  \beta_{TWI}*TWI + \beta_{Competition}*\sum _{j<20m} DBH_j^{\beta_{NCI}} * e^{-\alpha_{NCI}*d_{i,j}} + \beta_Gaps*\sum _{k<50m} S_k^\beta * e^{-\alpha*d_{i,k}}, \sigma) $$
...
```{stan Mall, output.var="Mall", echo=T, eval=F}
  ...
  parameters {
    ...
    real<upper=1> beta_TWI ;
    real<upper=1> beta_Competition ;
    real<lower=0, upper=1> alpha_NCI ;
    real<lower=0, upper=3> beta_NCI ;
    real<upper=1> beta_Gaps ;
    real<lower=0, upper=1> alpha_Idist ;
    real<lower=0, upper=3> beta_Idist ;
    real<lower=0> sigma ;
  }
  ...
  model {
    for(n in 1:N)
      LDMC[n] ~ normal((alpha_DBH*DBH[n])/(DBH[n] + beta_DBH) + beta_TWI*TWI[n] +  beta_Competition*NCI[n] + beta_Gaps*Idist[n], sigma) ;
  }
  generated quantities {
    real LDMCpred[N] ;
    for(n in 1:N)
      LDMCpred[n] = (alpha_DBH*DBH[n])/(DBH[n] + beta_DBH) + beta_TWI*TWI[n] + beta_Competition*NCI[n] + beta_Gaps*Idist[n] ;
  }
```
...
# save(fit, file = "./functional_save/MLDMCall.Rdata")
load("./functional_save/MLDMCall.Rdata")
...
cbind(
  ...
  geom_point(aes(y = LDMC, size = DBH0, aes = DBH0, col = Bark)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +

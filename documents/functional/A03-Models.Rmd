...
**Introduction in progress and under correction,  please ignore for the moment.**
...
Previous analysis highlighted a strong correlation between DBH and Dawkins in our dataset. Thus, we decided to analyze trait variation according to DBH, that depict here both light access and ontogeny effect ( see figure \@ref(fig:DBH)).
...
* $T_i$ is a functional trait of an individual i associated to a $DBH_i$ value ;
...
Local Basal Area (LBA) was used as a proxy of Neighborhood Competition Index and represented biotic interactions effect on traits. LBA can be already considered as a form of competition model which sets $DBH$ of neighbors within a specific area $\pi \times \delta_{max}^2$ around the individual as variable.
...
We explained leaf trait $T$ according to competition following a normal law with a linear model form around a Neighborhood Crowding Index (including both neighbor diameter and distance) :
...
First model form to be investigated is a model where phenotype would be explained by each submodel with the same strength through an additional model form. All constraints related in submodels remain true for full model building. Namely, a possibility of null response will have to be integrated, while avoiding to add non identifiable parameters.
...
* $\sigma_{DBH}$ is normal law standard error, and will, be chosen with positive values. 
...
* $\sigma_{DBH}$ and  $\sigma$ are normal law standard errors, and will be chosen with positive values;
...
In previous models, taxonomic levels were not integrated, but individuals displayed different trends depending on complex and species they belonged to. We chose to deal with taxonomic levels as random nested effects of three levels on several parameters from the full model: (i) variability of individuals between complexes ($C$ levels), (ii) variability of individuals within a complex and between species ($S_c$ level), and (iii) variability of individuals within a species ($I_{s}$ level). Consequently, the additional full model become:
...
See previous paragraphs for parameters definitions and interpretations.

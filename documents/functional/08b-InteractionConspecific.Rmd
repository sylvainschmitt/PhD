...
# Interaction Model conspecific

## Results

### Summary

```{r summarySpeciesIntCon}
...
```{r speciesInterceptIntCon, fig.cap="Model species effect for Intercept."}
lapply(fits, function(fit) 
...
```{r speciesDBHIntCon, fig.cap="Model species effect for $DBH_{90}$."}
lapply(fits, function(fit) 
  mcmc_intervals_data(as.array(fit,
                               pars = "betaDBH"))) %>% 
  bind_rows(.id = "model") %>% 
...
```{r speciesDBH2IntCon, fig.cap="Model species effect for DBH."}
preds %>% 
  mutate(Trait = recode_factor(Trait, invLA = "frac(1, LA)")) %>% 
...
```{r speciesTWIIntCon, fig.cap="Model species effect for TWI."}
lapply(fits, function(fit) 
  mcmc_intervals_data(as.array(fit,
                               pars = "betaTWI"))) %>% 
  bind_rows(.id = "model") %>% 
...
```{r speciesTWI2IntCon, fig.cap="Model species effect for DBH."}
preds %>% 
  mutate(Trait = recode_factor(Trait, invLA = "frac(1, LA)")) %>% 
...
```{r speciesNCIconIntCon, fig.cap="Model species effect for NCIcon."}
lapply(fits, function(fit) 
  mcmc_intervals_data(as.array(fit,
                               pars = "betaNCIcon"))) %>% 
  bind_rows(.id = "model") %>% 
...
```{r speciesNCIcon2IntCon, fig.cap="Model species effect for NCIcon."}
preds %>% 
  mutate(Trait = recode_factor(Trait, invLA = "frac(1, LA)")) %>% 
...
```{r speciesNCIheteroIntCon, fig.cap="Model species effect for NCIhetero"}
lapply(fits, function(fit) 
  mcmc_intervals_data(as.array(fit,
                               pars = "betaNCIhetero"))) %>% 
  bind_rows(.id = "model") %>% 
...
```{r speciesNCIhetero2IntCon, fig.cap="Model species effect for NCIhetero."}
preds %>% 
  mutate(Trait = recode_factor(Trait, invLA = "frac(1, LA)")) %>% 
  ggplot(aes(NCIcon, col = Species)) +
  geom_point(aes(y = Y), alpha = 0.2) +

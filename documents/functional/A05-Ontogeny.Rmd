...
model {
  alpha_c ~ normal(0, 10^6) ;
  betaDBH_c ~ normal(0, 10^6) ;
  alpha_s_tilde ~ normal(0, 1) ;
  betaDBH_s_tilde ~ normal(0, 1) ;
  sigmaIntercept ~ lognormal(0,1) ;
  sigmaDBH ~ lognormal(0,1) ;
  sigma ~ cauchy(0,5) ;
  Trait ~ normal(alpha_s[species] + betaDBH_s[species] .* DBH, sigma) ; // Likelihood
}
generated quantities {
  vector[N] Trait_pred ;
  Trait_pred = alpha_c[complex] + betaDBH_c[complex] .* DBH ; // Predictions
}
```
...
lapply(models, function(model)
  ...
  kable(caption = "log-likelihood for ontogenic model depending on model form.")
```

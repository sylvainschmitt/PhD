...
First model for BD variation in relation to environment.
...
```{r BDDBH}
data %>% 
  ggplot(aes(DBH, brBD, col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for DBH effect on BD.

$$BD \sim \mathcal{N} (\alpha + \beta* dbh, \sigma) $$
...
```{stan MdbhLinear, output.var="MdbhLinear", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      BD[n] ~ normal(alpha + beta*DBH[n], sigma) ;
  }
  generated quantities {
    real BDpred[N] ;
    for(n in 1:N)
      BDpred[n] = alpha + beta*DBH[n] ;
  }
```
...
#                             BD = data$brBD,
#                             DBH = data$DBH))
# save(fit, file = "./functional_save/ModelbrBDdbhLinear.Rdata")
load("./functional_save/ModelbrBDdbhLinear.Rdata")
...
cbind(
  data,
  pred = apply(as.matrix(fit, pars = "BDpred"), 2, mean),
  t(apply(as.matrix(fit, pars = "BDpred"), 2, quantile, probs = c(0.05, 0.95)))
) %>% 
  ggplot(aes(x = DBH)) +
  geom_point(aes(y = brBD)) +
...
$$BD \sim \mathcal{N} (\frac{dbh + \beta}{\alpha * dbh}, \sigma) $$
...
```{stan Mdbh, output.var="Mdbh", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      BD[n] ~ normal((DBH[n] + beta)/(alpha*DBH[n]), sigma) ;
  }
  generated quantities {
    real BDpred[N] ;
    for(n in 1:N)
      BDpred[n] = (DBH[n] + beta)/(alpha*DBH[n]) ;
  }
```
...
#                             BD = data$brBD,
#                             DBH = data$DBH))
# save(fit, file = "./functional_save/ModelbrBDdbh.Rdata")
load("./functional_save/ModelbrBDdbh.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
  kable(caption = "Summary table of the model dbh only")
```
...
```{r BDTWI}
data %>% 
  ggplot(aes(TWI, brBD, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for TWI effect on BD

$$BD \sim \mathcal{N} (\alpha + \beta* TWI, \sigma) $$
...
```{stan MTWI, output.var="MTWI", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      BD[n] ~ normal(alpha + beta*TWI[n], sigma) ;
  }
  generated quantities {
    real BDpred[N] ;
    for(n in 1:N)
      BDpred[n] = alpha + beta*TWI[n] ;
  }
```
...
#                             BD = data$brBD,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelbrBDTWI.Rdata")
load("./functional_save/ModelbrBDTWI.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
...
Topographic wetness index as significantly strong negative effect on BD.
...
```{r BDElevation}
data %>% 
  ggplot(aes(DEM, brBD, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for elevation effect on BD

$$BD \sim \mathcal{N} (\alpha + \beta* Elevation, \sigma) $$
...
```{stan MElevation, output.var="MElevation", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      BD[n] ~ normal(alpha + beta*Elevation[n], sigma) ;
  }
  generated quantities {
    real BDpred[N] ;
    for(n in 1:N)
      BDpred[n] = alpha + beta*Elevation[n] ;
  }
```
...
#                             BD = data$brBD,
#                             Elevation = data$DEM))
# save(fit, file = "./functional_save/ModelbrBDElevation.Rdata")
load("./functional_save/ModelbrBDElevation.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
...
Competition %>% 
  ...
  ggplot(aes(NCI, brBD, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$BD_i \sim \mathcal{N} (\alpha + \beta* \sum _{j<20m} DBH_j^{\beta_{NCI}} * e^{-\alpha_{NCI}*d_{i,j}} , \sigma) $$
...
```{stan Mcomp, output.var="Mcomp", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      BD[n] ~ normal(alpha + beta*NCI[n], sigma) ;
  }
  generated quantities {
    real BDpred[N] ;
    for(n in 1:N)
      BDpred[n] = alpha + beta*NCI[n] ;
  }
```
...
# save(fit, file = "./functional_save/ModelbrBDCompetition.Rdata")
load("./functional_save/ModelbrBDCompetition.Rdata")
...
cbind(
  ...
  ggplot(aes(x = NCI)) +
  geom_point(aes(y = BD, size = DBH)) +
  geom_text(aes(y = BD, label = idTreei)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed")
...
Gaps %>% 
  ...
  full_join(data) %>% 
  ggplot(aes(Idist, brBD, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
$$BD_i \sim \mathcal{N} (\sum _{j<20m} S_j^\beta * e^{-\alpha*d_{i,j}} , \sigma) $$
...
```{stan Mgaps, output.var="Mgaps", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      BD[n] ~ normal(alpha + beta*Idist[n], sigma) ;
  }
  generated quantities {
    real BDpred[N] ;
    for(n in 1:N)
      BDpred[n] = alpha + beta*Idist[n] ;
  }
```
...
# save(fit, file = "./functional_save/ModelbrBDGaps.Rdata")
load("./functional_save/ModelbrBDGaps.Rdata")
...
cbind(
  Gaps %>% 
    ...
    left_join(data),
  pred = apply(as.matrix(fit, pars = "brBDpred"), 2, mean),
  t(apply(as.matrix(fit, pars = "brBDpred"), 2, quantile, probs = c(0.05, 0.95)))
) %>% 
  ggplot(aes(x = Idisturb)) +
  geom_point(aes(y = brBD, size = DBH)) +
  geom_text(aes(y = brBD, label = idTree)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed")
...
data %>% 
  ...
  left_join(Gaps %>% 
              ...
              mutate(Distance = Distance/max(Distance)) %>% 
              summarise(Idist = sum(Area*exp(-Distance)))) %>% 
  dplyr::select(Bark, brBD, DBH, TWI, NCI, Idist) %>%
  mutate(DBH0 = DBH) %>% 
  reshape2::melt(id.vars = c("Bark", "brBD", "DBH0")) %>% 
  ggplot(aes(value, brBD, size = DBH0, aes = DBH0,
             col = Bark, group = NA)) +
  geom_point() +
...
$$BD \sim \mathcal{N} (\frac{dbh + \beta_{DBH}}{\alpha_{DBH} * dbh} +  \beta_{TWI}*TWI + \beta_{Competition}*\sum _{j<20m} DBH_j^{\beta_{NCI}} * e^{-\alpha_{NCI}*d_{i,j}} + \beta_Gaps*\sum _{k<50m} S_k^\beta * e^{-\alpha*d_{i,k}}, \sigma) $$
...
```{stan Mall, output.var="Mall", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      BD[n] ~ normal((DBH[n] + beta_DBH)/(alpha_DBH*DBH[n]) + beta_TWI*TWI[n] +  beta_Competition*NCI[n] + beta_Gaps*Idist[n], sigma) ;
  }
  generated quantities {
    real BDpred[N] ;
    for(n in 1:N)
      BDpred[n] = (DBH[n] + beta_DBH)/(alpha_DBH*DBH[n]) + beta_TWI*TWI[n] + NCI[n] + beta_Gaps*Idist[n] ;
  }
```
...
cbind(
  ...
  geom_point(aes(y = BD, size = DBH0, aes = DBH0, col = Bark)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +

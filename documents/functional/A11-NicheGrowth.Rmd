...
parameters <- lapply(fits, function(fit)
  ...
  select(Complex, Species, parameter, variable, value) %>% 
...
```{r NicheRelationIndividualGrowth, fig.cap="Individual growth - Niche relation."}
data %>% 
  mutate(AGRmaxind = apply(as.matrix(fit, pars = "AGRmaxind"), 2, mean)[as.numeric(as.factor(data$idTree))]) %>% 
  left_join(Individuals) %>% 
  select(Genus, Species, idTree, AGRmaxind, logSuitability) %>% 
  unique() %>% 
  mutate(SpeciesLong = paste(substr(Genus, 1, 1), Species)) %>% 
...
```{r NicheRelationVigor, fig.cap="Individual vigor - niche relation."}
data %>% 
  mutate(SpeciesLong = paste(substr(Genus, 1, 1), Species)) %>% 
  mutate(AGRpred = apply(as.matrix(fit, pars = "AGRpred"), 2, mean)) %>%
  group_by(idTree) %>% 
  ...
  left_join(Individuals) %>% 
  select(Genus, Species, idTree, Vigor, logSuitability) %>% 
  unique() %>% 
    filter(!is.na(logSuitability)) %>% 
  mutate(SpeciesLong = paste(substr(Genus, 1, 1), Species)) %>% 
...
```{stan modellogSuitability, output.var="Model", echo=F, eval=F, cache=F}
data {
  int<lower=1> N ; // Nb of observations
  vector<lower=0>[N] AGR ; // growth vector
  vector<lower=0>[N] dbh ; // dbh vector
...
data <- src_sqlite(file.path(path, "trees", "Paracou.sqlite")) %>%
  ...
  left_join(Individuals) %>% 
  ungroup() %>% 
  select(idTree, Genus, agr, dbh, logSuitability, Species) %>% 
...
```{r predicitonlogSuitability, fig.cap="Model predictions for logSuitability growth."}
data %>% 
  mutate(SpeciesLong = paste(substr(Genus, 1, 1), Species)) %>% 
  mutate(AGRpred = apply(as.matrix(fit, pars = "AGRpred"), 2, mean)) %>% 
  ggplot(aes(x = dbh, col = SpeciesLong, group = idTree)) + 
...
```{r logSuitabilityPosteriors, fig.cap="Model posteriors for logSuitability effect on growth."}

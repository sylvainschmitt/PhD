---
title: Ontogeny, topgraphy and competition shape individual-tree leaf functional traits independantly from species identity within Neotropical species complexes (Temporary)
...
* LMA, LT, CC seggregate species => Conservation/Acquisition ? orthogonal to LDMC, LA seggregating complexes
...
* ~ CC, and LDMC relatively stable within complex, LA, SLA and LT seemed to differ btw species with a gradient in each complex

## $DBH_{90}$ (Fig. \@ref(fig:DBH90))

* ~ strong effect on LA, SLA and LT but low or null effect on CC and LDMC
* ~ + DBH => -LA, -SLA, +LT => +Conservation & -Acquisition with age and light access ?

## TWI (Fig. \@ref(fig:TWI))

* ~ Globally more or less strong negative effect on CC, invLA, LMA, LDMC and LT *Symphonia* and *Escwilera* except for CC *Eschweilera* and for less abundant species within *Eschweilera* (*E. pedicellata*, *E. wachenheimii*, and *E. grandiflora* for SLA, LDMC and LT)
* ~ + TWI => + LA, +SLA, -LDMC, -LT, +/-CC (complex) => +Acquistion & -Conservation ?
* *S. sp1* more uncertain than *S. globulifera*
* LT *Symphonia* highly uncertain => Season effect ?

## NCI (Fig. \@ref(fig:NCI))

* ~ Always positive and non null
* ~ + NCI => + LA, +SLA, -LDMC, -LT, -CC => +Acquistion & -Conservation with increased competition ?

## Sensitivity (Fig. \@ref(fig:sensitivity))

* ~ relatively similar, DBH only seems to be more important than environment in shaping traits
* ~ TWI more important CC *Eschweilera*
* ~ NCI + TWI faible on LT *Symphonia* => Season effect ?

# Discussion

> To be developed in bullet points to highlight the take home message

> Ontogeny, topography and competition shape individual leaf traits independantly from species identity
...
preds %>%
  ...
  rename(DBH = Ddbh, TWI = Dtwi, NCI = Dcomp) %>% 

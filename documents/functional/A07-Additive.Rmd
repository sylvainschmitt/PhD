...
#   sampling(Model, chains = 2, data = x, include = F, save_warmup = F,
#            pars = c("NCIj", "alpha_s", "betaDBH_s", "betaTWI_s")))
# names(fits) <- traits
...
Leaf area $LA$ did not converged and will be fixed. In the meantime we show the posteriors of other models posteriors below.
...
```{r }
traits <- c("invSLA", "LDMC", "LT", "CC") # invLA did not converged and should be corrected
fits[["invLA"]] <- NULL
```
...
```{r Rsqaured}
lapply(as.list(traits), function(trait) 
  as.data.frame(fits[[trait]], pars = c("Trait_pred")) %>% 
    reshape2::melt() %>% 
    group_by(variable) %>% 
    summarise(value = mean(value)) %>% 
    ungroup() %>% 
    mutate(variable = gsub("]", "", variable, fixed = T)) %>% 
    separate(variable, c("variable", "index"), sep = "\\[") %>% 
    separate(index, c("obs", "complex"), sep = ",", convert = T) %>% 
    rename(Ypred = value) %>% 
    select(-variable) %>% 
    left_join(data.frame(obs = 1:mdata[[trait]]$N,
                         Y = mdata[[trait]]$Trait,
                         complex = mdata[[trait]]$complex),
              by = c("obs", "complex")) %>% 
    arrange(obs) %>% 
    filter(!is.na(Y)) %>% 
    mutate(trait = trait)) %>% 
  ...
  group_by(trait) %>% 
  mutate(Ymean = mean(Y)) %>% 
  mutate(SquaredResiduals = (Y - Ypred)^2) %>% 
  mutate(SquaredTotals = (Y - Ymean)^2) %>% 
  summarise(Rsquared = 1 - sum(SquaredResiduals)/sum(SquaredTotals)) %>% 
  kable(caption = "Models Rsquared", col.names = c("Trait", "$R^2$"))
```
...
student <- lapply(fits, function(fit) 
  ...
  left_join(gdata %>% 
              group_by(trait, parameter) %>% 
              summarise(x = mean(m)))
significance <- lapply(fits, function(fit) 
  as.data.frame(fit, pars = c("alpha_c", "betaDBH_c",
                                     "betaTWI_c", "betaComp_c"))) %>% 
  bind_rows(.id = "trait") %>%
  reshape2::melt(id.vars = "trait", variable.name = "parameter") %>% 
  ...
  mutate(parameter = recode_factor(parameter, betaDBH = "beta[DBH]", 
                                   betaTWI = "beta[TWI]", betaComp = "beta[Comp]")) %>% 
  group_by(trait, parameter, complex) %>% 
  summarise(pvalue = rank(c(0,value))[1]/(length(value)+1)) %>% 
  mutate(pvalue = ifelse(pvalue > 0.5, 1-pvalue, pvalue)) %>% 
...
ggplot(gdata, aes(x = trait, xend = trait, col = complex, fill = complex)) +
  ...
  geom_text(aes(x = trait, y = x, label = student,
            ...
            data = student) +
  geom_text(aes(x = trait, y = x, label = pvalue, fill = NA), 
...
significance <- lapply(fits, function(fit) 
  as.data.frame(fit, pars = c("alphaNCI"))) %>% 
  bind_rows(.id = "trait") %>%
  group_by(trait) %>% 
  summarise(pvalue = rank(c(0,alphaNCI))[1]/(length(alphaNCI)+1)) %>% 
  mutate(pvalue = ifelse(pvalue > 0.5, 1-pvalue, pvalue)) %>% 
  mutate(pvalue = ifelse(pvalue < 0.001, "***",
                          ifelse(pvalue < 0.01, "**",
                                 ifelse(pvalue < 0.05, "*",
...
ggplot(gdata, aes(x = trait, xend = trait)) +
  ...
  geom_segment(aes(y = l, yend = h), size = 2, alpha = 0.5) +
  coord_flip() +
  geom_text(aes(x = trait, y = m, label = pvalue), 
            position = position_nudge(x = -0.1),
            data = significance) +
  facet_wrap(~ parameter, labeller = label_parsed, scales = "free") +
  xaxis_title(F) +
...
significance <- lapply(fits, function(fit) 
  as.data.frame(fit, pars = c("sigma", "sigmaIntercept", 
                              "sigmaDBH", "sigmaTWI", "sigmaComp"))) %>% 
  bind_rows(.id = "trait") %>%
  reshape2::melt(id.vars = "trait", variable.name = "parameter") %>% 
  group_by(trait, parameter) %>% 
  summarise(pvalue = rank(c(0,value))[1]/(length(value)+1)) %>% 
...
list(
  DBH = lapply(as.list(traits), function(trait)
    cbind(trait = trait, 
          variable = "DBH",
          mu = as.numeric(scale(Individuals[!is.na(unlist(Individuals[,trait])),trait],center=F)),
          value = as.numeric(scale(Individuals[!is.na(unlist(Individuals[,trait])),"DBH"],center=F)),
          Individuals[!is.na(unlist(Individuals[,trait])),],
          muEschweilera = apply(as.matrix(fits[[trait]], 
                                          pars = paste0("Trait_predDBH[",
                                                        1:mdata[[trait]]$N,",1]")), 
                                2, mean),
          muSymphonia = apply(as.matrix(fits[[trait]], 
                                        pars = paste0("Trait_predDBH[",
                                                      1:mdata[[trait]]$N,",2]")), 
                              2, mean),
          muEschweilera5 = apply(as.matrix(fits[[trait]], 
                                           pars = paste0("Trait_predDBH[",
                                                         1:mdata[[trait]]$N,",1]")), 
                                 2, quantile, probs = 0.05),
          muSymphonia5 = apply(as.matrix(fits[[trait]], 
                                         pars = paste0("Trait_predDBH[",
                                                       1:mdata[[trait]]$N,",2]")), 
                               2, quantile, probs = 0.05),
          muEschweilera95 = apply(as.matrix(fits[[trait]], 
                                            pars = paste0("Trait_predDBH[",
                                                          1:mdata[[trait]]$N,",1]")), 
                                  2, quantile, probs = 0.95),
          muSymphonia95 = apply(as.matrix(fits[[trait]], 
                                          pars = paste0("Trait_predDBH[",
                                                        1:mdata[[trait]]$N,",2]")), 
                                2, quantile, probs = 0.95)
    )) %>% 
    bind_rows(),
  TWI = lapply(as.list(traits), function(trait)
    cbind(trait = trait, 
          variable = "TWI",
          mu = as.numeric(scale(Individuals[!is.na(unlist(Individuals[,trait])),trait],center=F)),
          value = as.numeric(scale(Individuals[!is.na(unlist(Individuals[,trait])),"TWI"],center=F)),
          Individuals[!is.na(unlist(Individuals[,trait])),],
          muEschweilera = apply(as.matrix(fits[[trait]], 
                                          pars = paste0("Trait_predTWI[",
                                                        1:mdata[[trait]]$N,",1]")), 
                                2, mean),
          muSymphonia = apply(as.matrix(fits[[trait]], 
                                        pars = paste0("Trait_predTWI[",
    ...
    bind_rows(),
  NCI = lapply(as.list(traits), function(trait)
    cbind(trait = trait, 
          variable = "NCI",
          mu = as.numeric(scale(Individuals[!is.na(unlist(Individuals[,trait])),trait],center=F)),
          value = apply(as.matrix(fits[[trait]], 
                                          pars = paste0("NCI[", 1:mdata[[trait]]$N,"]")), 
                                2, mean),
          Individuals[!is.na(unlist(Individuals[,trait])),],
          muEschweilera = apply(as.matrix(fits[[trait]], 
                                          pars = paste0("Trait_predNCI[",
                                                        1:mdata[[trait]]$N,",1]")), 
                                2, mean),
          muSymphonia = apply(as.matrix(fits[[trait]], 
                                        pars = paste0("Trait_predNCI[",
  ...
  bind_rows() %>% 
  ggplot(aes(x = value)) +
  geom_point(aes(y = mu, col = as.factor(Genus))) +
...
```{r posteriorsArea, eval=F}
gdata <- lapply(fits, function(fit) 
  mcmc_areas_data(as.array(fit),
                      regex_pars = c("alpha_c", "betaDBH_c",
                                     "betaTWI_c", "betaComp_c"))) %>% 
  bind_rows(.id = "trait") %>% 
  mutate(complex = gsub("([[:alpha:]])", "", parameter)) %>% 
...
student <- lapply(fits, function(fit) 
  ...
  left_join(gdata %>% 
              group_by(trait, parameter) %>% 
              summarise(x = mean(x), y = max(density)))
significance <- lapply(fits, function(fit) 
    as.data.frame(fit, pars = c("alpha_c", "betaDBH_c",
                                     "betaTWI_c", "betaComp_c"))) %>% 
  bind_rows(.id = "trait") %>%
  ...
  mutate(pvalue = ifelse(pvalue < 0.001, "***",
    ...
    left_join(gdata %>% 
              group_by(trait, parameter, complex) %>% 
              summarise(x = mean(x), y = max(density)))
ggplot(split(gdata, gdata$interval)$outer) +
  aes(x, trait, col = complex, fill = complex) +
  geom_vline(xintercept = 0, color = "gray90", size = 0.5) +
  ggridges::geom_ridgeline(mapping = aes(height = density/max(density)),
                           data = split(gdata, gdata$interval)$inner,
                           alpha = 0.5) +
  ggridges::geom_ridgeline(aes(height = density/max(density)), color = NA,
                           data = split(gdata, gdata$interval)$point) +
  ggridges::geom_ridgeline(mapping = aes(height = density/max(density)),
                           data = split(gdata, gdata$interval)$outer, 
                           fill = NA, alpha = 0.5) +
  geom_segment(aes(x = ll, xend = hh, yend = trait),
               data = split(gdata, gdata$interval)$outer %>%
                 group_by(trait, parameter, complex) %>%
                 summarise(ll = min(.data$x), hh = max(.data$x)) %>%
                 ungroup()) +
  geom_text(aes(x =x, y = trait, label = student,
                col = NA, fill = NA), position = position_nudge(y = -0.1),
            data = student) +
  geom_text(aes(x =x, y = trait, label = pvalue,
                col = "Eschweilera", fill = NA), 
            position = position_nudge(y = -0.2),
            data = split(significance, significance$complex)$Eschweilera) +
  geom_text(aes(x =x, y = trait, label = pvalue,
                col = "Symphonia", fill = NA), 
            position = position_nudge(y = -0.3),
            data = split(significance, significance$complex)$Symphonia) +
  facet_wrap(~ parameter, labeller = label_parsed, scales = "free") +
  xaxis_title(F) +
  yaxis_title(F)
  # + ggsave("./functional_save/AdditivePosteriors.png",

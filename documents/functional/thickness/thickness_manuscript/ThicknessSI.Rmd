...
read_tsv("../thickness_save/data.tsv") %>% 
  dplyr::select(LT, DBH, T55, T30, Hr55, Hr30, Patm, windaspect, windspeed, 
...
lapply(fits, mcmc_intervals_data, pars = "lp__") %>% 
  bind_rows(.id = "model") %>% 
  mutate(random = ifelse(m < 0, "no individual random effect", 
                         "with individual random effect")) %>% 
  mutate(model = recode(model,
                        "rew"= "REW",
                        "rewspecies" = "REW + Species",
                        "rewindividual" = "REW",
                        "rewspeciesindividual" = "REW + Species",
                        "rewdbh" = "REW + DBH",
                        "rewdbhspecies" = "REW + DBH + Species",
                        "rewdbhindividual" = "REW + DBH",
                        "rewdbhspeciesindividual" = "REW + DBH + Species")) %>% 
  ggplot(aes(x = model, 
             xend = model,
             color = model, 
             fill = model)) +
  ...
  coord_flip() +
  facet_wrap(~ random, scales = "free_x") +

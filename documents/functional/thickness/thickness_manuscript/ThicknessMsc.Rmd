...
spanning the natural distribution of DBH and topographic habitats.
...
We used a linear mixed models with REW, DBH and species fixed effects combined with individuals random effect to explain leaf thickness among several tested models (Mat. Sup. S2).

Leaf thickness $LT_{l,i,s}$ for leaf $l$ belonging individual $i\in[1;I]$ in species $s\in[1;S]$ 
was inferred with a normal distribution with the following formula:
...
and $\beta_{REW}$ and $\beta_{DBH}$ are the slope of REW and DBH effects.
We calculated conditional and marginal $R^2$ [@Nakagawa2013] and used them to partition variance between fixed and random effects.
...
(Tab. \@ref(tab:pars) and Fig. \@ref(fig:preds))

Variance partitioning showed the importance of relative extractable water (REW) and tree diameter (DBH) in leaf thickness variation (Tab. \@ref(tab:pars) and Fig. \@ref(fig:preds)).
After individual random effects (46%), REW and DBH explained respectivelly 26% and 15% of the leaf thickness variation, and represented 90% of the total leaf thickness variation with species and individuals effects.
REW showed a strong negative effect on leaf thickness (-0.49, Tab. \@ref(tab:pars)), and DBH showed a positive effect (0.37, Tab. \@ref(tab:pars)).
...
data <- read_tsv("../thickness_save/data.tsv") %>% 
  ...
  mutate(Species = recode(Species,
                        ...
                        "sp.1" = "S. sp1"))
load("../thickness_save/fits.Rdata")
as.data.frame(fits$RxSDSI, pars = "predicted") %>% 
  reshape2::melt(NULL) %>% 
  group_by(variable) %>% 

...
```{r posteriorsBeta}
gdata <- lapply(as.list(fits), function(fit)
  mcmc_intervals_data(as.array(fit), regex_pars = c("betaDBH_sd_c",
                                          "betaTWI_sd_c", "betaComp_c"))) %>% 
  bind_rows(.id = "trait") %>% 
...
student <- lapply(fits, function(fit) 
  as.data.frame(fit, pars = c("betaDBH_sd_c",
                              "betaTWI_sd_c", "betaComp_c"))) %>% 
  bind_rows(.id = "trait") %>%
...
```{r DBH90}
lapply(as.list(fits), function(fit)
  mcmc_intervals_data(as.array(fit), regex_pars = c("DBH90"))) %>% 
  bind_rows(.id = "trait") %>% 
  mutate(parameter = recode_factor(parameter, alphaNCI = "alpha[NCI]")) %>% 

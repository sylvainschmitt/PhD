...
generated quantities {
  ...
  for(c in 1:C)
    Trait_pred[,c] = alpha_c[c] * (DBH ./ (betaDBH_c[c] + DBH)) + betaTWI_c[c]*TWI ; // Predictions
  for(c in 1:C)
    Trait_predDBH[,c] = alpha_c[c] * (DBH ./ (betaDBH_c[c] + DBH)) + betaTWI_c[c]*mean(TWI) ;
   for(c in 1:C)
    Trait_predTWI[,c] = alpha_c[c] * (mean(DBH) ./ (betaDBH_c[c] + mean(DBH))) + betaTWI_c[c]*TWI ;
}
...
# names(fits) <- traits
# save(fits, file = "./functional_save/AddOntoAbiot.Rdata")
...
```{r , eval=F, fig.cap="Markov chains trace plot after warmup for intermediate data.",fig.height=12,fig.width=12}
...
```{r , eval=F, fig.cap="Markov chains pairs plot after warmup  for intermediate data.",fig.height=20,fig.width=20}
...
fits <- lapply(mdata, function(x) sampling(Model, chains = 2, data = x))
names(fits) <- traits
save(fits, file = "./functional_save/HierOntaAbiot2.Rdata")
#load("./functional_save/HierOntaAbiot2.Rdata")
...
```{r , eval=F, fig.cap="Markov chains pairs plot after warmup  for intermediate data.",fig.height=8,fig.width=8}
mcmc_pairs(as.array(fits$LDMC), regex_pars = pars)
```
...
# Hierarchical with biotic effect

...
traits <- googlesheets::gs_title("Measures") %>% 
  ...
  mutate(LT = as.numeric(LT), LT = as.numeric(LT))
```
...
First model for LT variation in relation to environment.
...
```{r LTDBH}
data %>% 
  ggplot(aes(DBH, LT, col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used an inverted Michealis Menten equation for DBH effect on LT.

$$LT \sim \mathcal{N} (\frac{dbh + \beta}{\alpha * dbh}, \sigma) $$
...
```{stan Mdbh, output.var="Mdbh", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LT[n] ~ normal((alpha*DBH[n])/(DBH[n] + beta), sigma) ;
  }
  generated quantities {
    real LTpred[N] ;
    for(n in 1:N)
      LTpred[n] = (alpha*DBH[n])/(DBH[n] + beta) ;
  }
```
...
#                             LT = data$LT,
#                             DBH = data$DBH))
# save(fit, file = "./functional_save/ModelLTdbh.Rdata")
load("./functional_save/ModelLTdbh.Rdata")
...
cbind(
  data,
  pred = apply(as.matrix(fit, pars = "LTpred"), 2, mean),
  t(apply(as.matrix(fit, pars = "LTpred"), 2, quantile, probs = c(0.05, 0.95)))
) %>% 
  ggplot(aes(x = DBH)) +
  geom_point(aes(y = LT)) +
...
We will used a linear equation for DBH effect on LT.

$$LT \sim \mathcal{N} (\alpha + \beta* dbh, \sigma) $$
...
```{stan MdbhLinear, output.var="MdbhLinear", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LT[n] ~ normal(alpha + beta*DBH[n], sigma) ;
  }
  generated quantities {
    real LTpred[N] ;
    for(n in 1:N)
      LTpred[n] = alpha + beta*DBH[n] ;
  }
```
...
#                             LT = data$LT,
#                             DBH = data$DBH))
# save(fit, file = "./functional_save/ModelLTdbhLinear.Rdata")
load("./functional_save/ModelLTdbhLinear.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
  kable(caption = "Summary table of the model dbh only")
```
...
```{r LTTWI}
data %>% 
  ggplot(aes(TWI, LT, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for TWI effect on LT.

$$LT \sim \mathcal{N} (\alpha + \beta* TWI, \sigma) $$
...
```{stan MTWI, output.var="MTWI", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LT[n] ~ normal(alpha + beta*TWI[n], sigma) ;
  }
  generated quantities {
    real LTpred[N] ;
    for(n in 1:N)
      LTpred[n] = alpha + beta*TWI[n] ;
  }
```
...
#                             LT = data$LT,
#                             TWI = data$TWI))
# save(fit, file = "./functional_save/ModelLTTWILinear.Rdata")
load("./functional_save/ModelLTTWILinear.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
...
Topographic wetness index as a significant negative effect on LT.
...
```{r LTElevation}
data %>% 
  ggplot(aes(DEM, LT, size = DBH, aes = DBH,
             col = Bark, group = NA)) +
  geom_point() +
  geom_smooth()
```
...
We will used a linear equation for elevation effect on LT.

$$LT \sim \mathcal{N} (\alpha + \beta* Elevation, \sigma) $$
...
```{stan MElevation, output.var="MElevation", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LT[n] ~ normal(alpha + beta*Elevation[n], sigma) ;
  }
  generated quantities {
    real LTpred[N] ;
    for(n in 1:N)
      LTpred[n] = alpha + beta*Elevation[n] ;
  }
```
...
#                             LT = data$LT,
#                             Elevation = data$DEM))
# save(fit, file = "./functional_save/ModelLTElevation.Rdata")
load("./functional_save/ModelLTElevation.Rdata")
pars <- c("alpha", "beta", "sigma")
broom::tidyMCMC(fit, pars = c(pars, "lp__"), droppars = NULL, rhat = T) %>% 
...
data %>% 
  ...
  dplyr::select(Bark, LT, DBH, TWI, NCI, Idist) %>%
  mutate(DBH0 = DBH) %>% 
  reshape2::melt(id.vars = c("Bark", "LT", "DBH0")) %>% 
  ggplot(aes(value, LT, size = DBH0, aes = DBH0,
             col = Bark, group = NA)) +
  geom_point() +
...
$$LT \sim \mathcal{N} (\frac{dbh + \beta_{DBH}}{\alpha_{DBH} * dbh} +  \beta_{TWI}*TWI + \beta_{Competition}*\sum _{j<20m} DBH_j^{\beta_{NCI}} * e^{-\alpha_{NCI}*d_{i,j}} + \beta_Gaps*\sum _{k<50m} S_k^\beta * e^{-\alpha*d_{i,k}}, \sigma) $$
...
```{stan Mall, output.var="Mall", echo=T, eval=F}
  ...
  model {
    for(n in 1:N)
      LT[n] ~ normal((DBH[n] + beta_DBH)/(alpha_DBH*DBH[n]) + beta_TWI*TWI[n] +  beta_Competition*NCI[n] + beta_Gaps*Idist[n], sigma) ;
  }
  generated quantities {
    real LTpred[N] ;
    for(n in 1:N)
      LTpred[n] = (DBH[n] + beta_DBH)/(alpha_DBH*DBH[n]) + beta_TWI*TWI[n] + NCI[n] + beta_Gaps*Idist[n] ;
  }
```
...
save(fit, file = "./functional_save/MLTall.Rdata")
load("./functional_save/MLTall.Rdata")
...
cbind(
  ...
  geom_point(aes(y = LT, size = DBH0, aes = DBH0, col = Bark)) +
  geom_line(aes(y = pred), col = "red") +
  geom_ribbon(aes(ymin = `5%`, ymax = `95%`), color = 'red', alpha = 0.2) +
  geom_line(aes(y = `5%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +
  geom_line(aes(y = `95%`), col = "red", alpha = 1, size = 0.5, linetype = "dashed") +

...
```{r LocalBasalAreaIndex, fig.cap='Boxplot of Dawkins and LBA'}
Comp <- Competition %>% 
  group_by(idTreei) %>% 
...
```
Boxplot between Dawkins and Local Basal Area (figure \@ref(fig:LocalBasalAreaIndex)) do not display any significant correlation between the two indexes. Whatever the Dawkins index is, individuals range almost in the same values, from 0.1 up to 0.5 m/ha.
...
```
The two graphs ( figure \@ref(fig:pairs) and \@ref(fig:corrplot)) show the correlations between environmental and biotic descriptors.
The first graph (figure \@ref(fig:pairs)) clearly shows that DEM and RelativeElevation, Slope and TRI respectively have a strong positive correlation.
...
```{r PCA, fig.cap="Principal compoonent analysis plot", eval=F}
...
```{r PCAfviz, fig.cap="Principal component analysis plot on two first plans"}
pca <- princomp(~ DEM + RelativeElevation + Slope + Curvature + 
                Aspect + TRI + TWI, env, cor = T)
...
```
Principal component analysis of environmental descriptors (figure \@ref(fig:PCAfviz)) is presented on the two main plans: Comp1 / Comp2 & Comp1 / Comp3.
...
The opposition of increasing RelativeElevation and DEM with decreasing Slope and TRI draw the second axis of PCA, which accounts for 21,4% of the total variance.
In addition, the first plan of environmental descriptors PCA show a strong correlation between RelativeElevation and DEM, and between Slope and TRI.
As far as Curvature and Aspect are very weakly represented on first PCA plan, drawing second PCA plan provide new insights on variable correlations. On second PCA plan, 58,5% of the total is conserved. The first axis is mainly supported by increasing TWI opposed to RelativeElevation, DEM, Slope and TRI, which correlations are strongly displayed on this axis. Furthermore, the second axis is strongly related to shrinking Curvature and Aspect. In addition the second axis display strong correlation between Curvature and Aspect, which are also decoupled from TWI, DEM, RelativeSlope, Slope, TRI.

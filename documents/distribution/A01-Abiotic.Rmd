...
```{stan Mtest, output.var="Mtest", echo=T, eval=F}
  ...
  model {
    ...
    for(n in 1:N)
      target += weights[n]*beta_lpdf(Presence[n] | (alpha + beta*TWI[n])*phi, (1.0 - (alpha + beta*TWI[n]))*phi) ;
  }
...
```{r, eval=F}
a = 0.517754 ; b = 0.0884929 ; c = 2.70395
mu = a+b
phi = c
(A = mu*phi)
(B = (1-mu)*phi)
...
```{r Cabiotic1Sympho, fig.cap="Model parameters posterior.", eval=F}
...
```{r Tabiotic1Sympho,fig.cap="Markov chains trace plot after warmup.", eval=F}
cowplot::plot_grid(plotlist = lapply(as.list(species), function(sp)
...
```{r 2abiotic1Sympho, fig.cap="Markov chains pairs plot after warmup.", eval=F}
mcmc_pairs(as.array(fits[[1]]), pars = c(pars, "lp__"))
```

```{r Yabiotic1Sympho, fig.cap="Predictions.", eval=F}
cowplot::plot_grid(plotlist = lapply(as.list(species), function(sp)
  cbind(datam,

...
vroom::vroom("modelling_save/simulations.tsv") %>% 
  ...
  reshape2::dcast(variable + metric + metric.value ~ parameter) %>% 
   mutate(variable = recode(variable, "nci" = "NCI", "topo" = "Topography")) %>% 
  group_by(variable, metric, Htopo, Hnci) %>% 
  summarise_all(max) %>% 
  ggplot(aes(x = Htopo, y = Hnci, z = 1-metric.value)) +
  geom_contour() +
  # geom_tile() +
  xlab(expression(H[topographie])) + ylab(expression(H[nci])) +
  viridis::scale_fill_viridis("Adaptation\nlocale")
```

```{r dgtwoHtopoHnci, fig.cap="Local adaptation (Dg, low value correspond to locally addapted trees) for topography and NCI depending on heritability to NCI (Hnci) and heritability to topography (Htopo)."}
data <- vroom::vroom("modelling_save/simulations.tsv") %>% 
  filter(timestep == 60) %>% 
  dplyr::select(-timestep) %>% 
  mutate(parameter.value = round(parameter.value, 1)) %>% 
  filter(metric == "Dg", variable == "topo") %>% 
  ...
  reshape2::dcast(variable + metric + metric.value ~ parameter) %>% 
   mutate(variable = recode(variable, "nci" = "NCI", "topo" = "Topography")) %>% 
  group_by(variable, metric, Htopo, Hnci) %>% 
  summarise(m = mean(metric.value), q5 = quantile(metric.value, 0.05), q95 = quantile(metric.value, 0.95))%>% 
  reshape2::melt(c("variable", "metric", "Htopo", "Hnci"), variable.name = "quantile")
  
 library("scatterplot3d")
scatterplot3d(x = data$Htopo, y = data$Hnci, z = 1 - data$value, 
              pch = 16, color = c("#999999", "#E69F00", "#56B4E9")[as.numeric(as.factor(data$quantile))],
              xlab = expression(H[topographie]), ylab = expression(H[NCI]), zlab = "Adaptation locale", 
              box = T, type="h", )
...
data <- vroom::vroom("modelling_save/simulations.tsv") %>% 
  ...
  group_by(variable, metric, Htopo, Hnci) %>% 
  summarise(m = mean(metric.value), q5 = quantile(metric.value, 0.05), q95 = quantile(metric.value, 0.95)) %>% 
  reshape2::dcast(Htopo ~ Hnci, value.var = "m") %>% 
  column_to_rownames("Htopo")


library(plotly)
# volcano is a numeric matrix that ships with R
fig <- plot_ly(z = ~ 1 - as.matrix(data)) %>% add_surface(
  contours = list(
    z = list(
      show=TRUE,
      usecolormap=TRUE,
...
fig <- plot_ly(z = ~1 - as.matrix(data))
fig <- fig %>% add_surface()

...
*beware* $t - (t-1)$ can be longer than one year for censuses separated by more than one year.
...
\delta NCI \sim \left\{
    \begin{array}{ll}
        P(\delta NCI \leq 0) \sim \mathcal B(logit^{-1}(\alpha + \beta.NCI)) |\alpha=-1.32,\beta=0.003 \\
...
```
```{r fideltaNCI, fig.cap="Results of the model with uncentered data for 10^4 observations."}
# mdata <- nci %>% 
...
Using inferred parameters $\alpha$, $\beta$, $\mu_{\delta NCI > 0}$, $\sigma_{\delta NCI > 0}$, and $\lambda_{\delta NCI \leq 0}$,
we successfully simulated back $\delta NCI$ spatial distribution (Fig. \@ref(fig:simspdNCIdist)).
Using inferred parameters $\mu_{NCI}$, and $\sigma_{NCI}$,
we successfully simulated back $NCI$ spatial distribution (Fig. \@ref(fig:simspNCIdist)).
...
We first thought to relate time steps to forest gap dynamics,
defining the time step as the typical time between two treefalls.
But in the end defining major treefalls is not that simple, 
and we used a different method to simulated NCI.
Consequently, we should find another method to define time steps,
as our current definition of death rate and deltaNCI is per year.
The idea is to avoid simulating tree growth and DBH,
...
and thus decided to use a time step $d_{time}$ of 30 years corresponding to a mortality of 32% (Fig. \@ref(fig:mortalityovertime)).
...
data.frame(Years = 1:500) %>% 
  ...
  scale_x_sqrt() +
  ggtitle("P(death over 30 years | Pdeath = 0.013) = 0.32467227")
```

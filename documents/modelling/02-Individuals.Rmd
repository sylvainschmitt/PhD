...
Simulations start with **environment**, **genotypes** and **phenotypes** initialisation, and follows with a cycle of four steps repeated over time:
...
The environmental mosaic where individuals grow is defined by a square matrix of size $grid\times grid$. 
Environment is generated with: (1) a gradient product, (2) a topography generator, or (3) a forest gap dynamics generator. Currently only the first method, a gradient product, is implemented. 
The environmental matrix varies in a grid defined by the product of the vectors $(−E_{lim},E_{lim})$ (Fig. \@ref(fig:env)).

```{r env, fig.cap="Example of environmental matrix obtained with gradient product."}
plotEnv(simulator(50,1))
```

...
count_SNP <- data %>% 
  dplyr::select(Is_in_CDS, Is_not_synonymous, Is_condition_specific) %>% 
...
goe_gene <- data %>% 
  dplyr::select(gene_id, width, id, go_id) %>% 
  unique() %>% 
  group_by(gene_id, id) %>% 
...
unsel <- data %>% 
  filter(grepl("RNA", go_name) | 
           grepl("ribo", go_name) | 
           grepl("mito", go_name) | 
           grepl("chloro", go_name)) %>% 
  dplyr:m:select(transcript_id, gene_id, go_name, width)

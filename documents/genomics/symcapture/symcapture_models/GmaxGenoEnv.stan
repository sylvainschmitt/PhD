...
transformed parameters {
  ...
  vector<lower=0>[I] thetai1 = exp(log(A*a + betaNCI*NCI + betaTWI*TWI) + sigmaR*epsilon_i) ; 
...
generated quantities{
  ...
  real Vnci = variance(log(A*a + betaNCI*NCI + betaTWI*TWI) - log(A*a + betaTWI*TWI)) ;
  real Vtwi = variance(log(A*a + betaNCI*NCI + betaTWI*TWI) - log(A*a + betaNCI*NCI)) ;
  real Vr = variance(to_vector(log(thetai1)) - log(A*a + betaNCI*NCI + betaTWI*TWI)) ;
}

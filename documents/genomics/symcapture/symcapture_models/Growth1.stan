...
parameters {
  vector<lower=0.1,upper=10>[G] Gmax ; // potential growth
  vector<lower=0,upper=200>[G] Dopt ; // optimal diameter
  vector<lower=0.1,upper=10>[G] Ks ; // kurtosis
  real<lower=0,upper=10> sigma ; // variance
}
model {
  log(AGR+1) ~ normal(Gmax[gp] .* exp(-0.5*(log(DBH ./ Dopt[gp]) ./ Ks[gp]).*(log(DBH ./ Dopt[gp]) ./ Ks[gp])), sigma) ; // likelihood
}

...
parameters {
  ...
  vector<lower=-10,upper=10>[I] betaAREWi ; // individual AREW slope
  real<lower=0,upper=2> sigmaAREWi ; // individual AREW slope variance
  ...
  vector<lower=-10,upper=10>[I] betaNCIi ; // individual NCI slope
  real<lower=0,upper=2> sigmaNCIi ; // individual NCI slope variance
...
model {
  ...
  betaAREWi ~ normal(betaAREWg[indingp], sigmaAREWi) ;
  betaNCIi ~ normal(betaNCIg[indingp], sigmaNCIi) ;
  ...
  log(AGR+1) ~ normal(Gmaxi[ind] .* exp(-0.5*(log(DBH ./ Dopti[ind]) ./ Ksi[ind]).*(log(DBH ./ Dopti[ind]) ./ Ksi[ind])) .* exp(-betaTWIg[gp] .* TWI) .* exp(-betaAREWi[ind] .* AREW) .* exp(-betaNCIi[ind] .* NCI), sigma) ; // likelihood
}

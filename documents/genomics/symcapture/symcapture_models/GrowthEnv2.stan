...
parameters {
  ...
  matrix[G, 4] beta ; // gene pool environmental effects
  matrix[I, 4] betai ; // gene pool environmental effects
  ...
  real<lower=0,upper=1> sigmabeta[4] ; // individual environmental effects variance
...
model {
  ...
  for(i in 1:4)
    betai[,i] ~ normal(beta[indingp,i], sigmabeta[i]) ;
  log(AGR+1) ~ normal(Gmaxi[ind] .* exp(-0.5*(log(DBH ./ Dopti[ind]) ./ Ksi[ind]).*(log(DBH ./ Dopti[ind]) ./ Ksi[ind])) .* exp(-betai[ind,1] .* TWI) .* exp(-betai[ind,2] .* AUC) .* exp(-betai[ind,3] .* logNCI) .* exp(-betai[ind,4] .* logANCR), sigma) ; // likelihood
}

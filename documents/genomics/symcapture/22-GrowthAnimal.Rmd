...
location <- log(0.54)
scale <-  0.66
# scale <-  sqrt(log(1 + (sd^2 / mode^2)))

ggplot(data.frame(x = seq(from = -1, to = 2,length.out = 100), 
                  y = seq(from = 0, to = 1,length.out = 100)), 
       aes(x = x, y = y)) +
  geom_vline(xintercept = mode, linetype = "dashed", color = "grey") +
  geom_vline(xintercept = as.vector(sapply(mode, function(x) x + c(-sd, +sd))), 
             linetype = "dotted", color = "grey") +
  stat_function(aes(col = "lognormal"), fun = function(.x)
    dlnorm(.x, meanlog = location[1], sdlog = scale[1]))
...
gkdata <- function(
  ...
  Gmaxi <- exp(log(Gmax[pop]) + 
                 sigmaG[1]*as.vector(t(chol(K)) %*% rnorm(I)) + 
                 sigmaR[1]*rnorm(I))
  Dopti <- exp(log(Gmax[pop]) + 
                 sigmaG[1]*as.vector(t(chol(K)) %*% rnorm(I)) + 
                 sigmaR[1]*rnorm(I))
  Ksi <- exp(log(Gmax[pop]) + 
                 sigmaG[1]*as.vector(t(chol(K)) %*% rnorm(I)) + 
                 sigmaR[1]*rnorm(I))
...
$$DBH_{y=today,p,i} - DBH_{y=y0,p,i} \sim \mathcal{logN} ( \\ log(\sum _{y=y_0} ^{y=today} e^{log(Gmax_p) + \sigma^2_{R1}.Gmax_i}.exp(-\frac12.[\frac{log(\frac{DBH_{y,p,i}}{Dopt_p})}{Ks_p}]^2)), \sigma^2) \\ Gmax_i \sim \mathcal N(0, 1) $$

```{r gamx1Data, fig.cap="Data used for the individual growth model combined with population animal model."}
# mdata <- gdata(I = 372, P = 1, Gmax = 0.54, sigmaR = c(0.5, 0, 0), sigma = 0.2)
...
# save(mdata, fitGmax1, file = file.path("symcapture_save", "gmax.Rdata"))
load(file = file.path("symcapture_save", "gmax.Rdata"))
...
```{r gmax1Tab}
broom::tidyMCMC(fitGmax1, pars = c("theta",
                                   "sigmaR", "sigma", "lp__"), 
...
# mcmc_pairs(as.array(fitGmax1, pars = c("theta", "sigmaR", "sigma")),
#            np = nuts_params(fitGmax1))
# mcmc_scatter(fitGmax1, pars = c("sigmaR", "sigma"))
```
...
mdata <- gdata(I = 100, P = 1, Gmax = 0.54, Dopt = 0.27, Ks = 0.65,
               sigmaR = c(0.3, 0.3, 0.3), sigma = 0.05)
...
ggplot(data.frame(DBH = 0:200, AGR = 2), aes(x = DBH, y = AGR)) +
  ...
  stat_function(aes(col = "Obtained"), fun = function(.x) 
    median(as.array(fitGrowth1, "theta[1,1]"))*
      exp(-.5*(log(.x/median(as.array(fitGrowth1, "theta[1,2]")))/median(as.array(fitGrowth1, "theta[1,3]")))^2))
...
mdata <- gkdata(Fam = 10, P = 1, Gmax = 0.65, Dopt = 0.27, Ks = 0.65,
                sigmaR = c(0.3, 0, 0), 
                sigmaG = c(0.15, 0, 0),
                sigma = 0.1)
gmaxgeno <- stan_model("./symcapture_models/GmaxGeno.stan")
fitGmaxGeno1 <- sampling(gmaxgeno, chains = 2, data = mdata$mdata, save_warmup = F,
                          control = list(adapt_delta = 0.9, max_treedepth = 12),
                          include = F, pars = c("DBH", "theta_raw", "thetai_raw", "a_raw",
                                                "sigmaR_raw", "sigmaG_raw", "Gmaxi", "a"))
save(mdata, fitGmaxGeno, file = file.path("symcapture_save", "gmaxgeno1.Rdata"))
...
mcmc_pairs(as.array(fitGmaxGeno, pars = c("sigmaR", "sigma")))
```
...
$$DBH_{y=today,p,i} - DBH_{y=y0,p,i} \sim \mathcal{logN} (log(\sum _{y=y_0} ^{y=today} Gmax_i.exp(-\frac12.[\frac{log(\frac{DBH_{y,p,i}}{Dopt_i})}{Ks_i}]^2)), \sigma)$$
$$\mid \forall \theta \in [Gmax, Dopt,Ks], \theta_i \sim \mathcal {logN} (log(\theta_p+A.a_\theta), \sigma^2_{R_{\theta}}),a_\theta\sim\mathcal N(0,\sigma_{G_\theta})$$

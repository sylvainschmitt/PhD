...
```{r TWIAnimal, fig.cap="Proportion of Variance Explained (PVE) by TWI."}
Y <- trees %>%
  mutate(popNum = as.numeric(as.factor(pop))) %>%
  dplyr::select(FID, IID, TWI, pop, popNum) %>%
  mutate(TWI = TWI/sd(TWI))
...
# fitsAnimal$TWI <- lapply(Yl, function(pop)
#   sampling(model, chains = 2, save_warmup = F, include = F, pars = "a",
#            control = list(adapt_delta = 0.99),
#            data = list(N = nrow(pop), Y = pop$TWI, K = K[pop$IID,pop$IID])))
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalModels.Rdata'))
load(file.path("symcapture_save", 'AnimalModels.Rdata'))
cowplot::plot_grid(plotlist = lapply(fitsAnimal$TWI, mcmc_combo, pars = c("sigmaG", "sigmaR")), 
                   labels = names(fitsAnimal$TWI))
lapply(fitsAnimal$TWI, broom::tidyMCMC,  pars = c("h2"), conf.int = T, rhat = T, estimate.method = "median") %>% 
...
```{r TWIAnimalPopMeanVar, fig.cap="Proportion of Variance Explained (PVE) by TWI."}
...
#   summarise(sigmaY = var(TWI)) %>%
#   dplyr::select(sigmaY) %>% unlist()
# model <- stan_model("./symcapture_models/AnimalPopMeanVar.stan")
# fitsAnimal$TWI <- sampling(model, chains = 2, save_warmup = F, include = F, pars = c("a", "G"),
#                             control = list(adapt_delta = 0.9),
#                             data = list(J = 1, N = nrow(Y), Y = Y$TWI, sigmaY = sigmaY,
...
mcmc_combo(as.array(fitsAnimal$TWI ,  pars = c("sigmaP", "sigmaG", "sigmaR")))
broom::tidyMCMC(fitsAnimal$TWI,  pars = c("sigmaP", "h2"), conf.int = T, rhat = T)
```
...
```{r NCIAnimal, fig.cap="Proportion of Variance Explained (PVE) by NCI"}
Y <- trees %>%
  mutate(popNum = as.numeric(as.factor(pop))) %>%
  dplyr::select(FID, IID, NCI, pop, popNum) %>%
  mutate(NCI = NCI/sd(NCI))
fam <- read_delim(file.path(path, "gemma", "Gmax0", "paracou3pop.fam"), delim = " ", 
                  col_names = c("FID", "IID", "PAT", "MAT", "SEX", "PHENOTYPE"))
K <- read_tsv(file.path(path, "gemma", "Gmax0", "paracou3pop.kinship.sXX.txt"), 
              col_names = fam$IID) %>% 
...
#           globuliferaTypeRegina = filter(Y, pop == "globuliferaTypeRegina"))
# model <- stan_model("./symcapture_models/Animal.stan")
# fitsAnimal$NCI <- lapply(Yl, function(pop)
#   sampling(model, chains = 2, save_warmup = F, include = F, pars = "a",
#            control = list(adapt_delta = 0.99),
#            data = list(N = nrow(pop), Y = pop$NCI, K = K[pop$IID,pop$IID])))
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalModels.Rdata'))
load(file.path("symcapture_save", 'AnimalModels.Rdata'))
cowplot::plot_grid(plotlist = lapply(fitsAnimal$NCI, mcmc_combo, pars = c("sigmaG", "sigmaR")), 
                   labels = names(fitsAnimal$NCI))
lapply(fitsAnimal$NCI, broom::tidyMCMC,  pars = c("h2"), conf.int = T, rhat = T, estimate.method = "median") %>% 
  bind_rows(.id = "Population")
```
...
```{r NCIAnimalPopMeanVar, fig.cap="Proportion of Variance Explained (PVE) by NCI"}
# Karray <- sapply(1:max(Y$popNum), function(p){
#   Kp = K[Y$IID,Y$IID]
#   for(n in 1:nrow(Y)) {
#     if(Y$popNum[n] != p) {
...
# dim(Karray) <- c(nrow(Y), nrow(Y), max(Y$popNum))
# sigmaY <- group_by(Y, popNum) %>%
#   summarise(sigmaY = var(NCI)) %>%
#   dplyr::select(sigmaY) %>% unlist()
# model <- stan_model("./symcapture_models/AnimalPopMeanVar.stan")
# fitsAnimal$NCI <- sampling(model, chains = 2, save_warmup = F, include = F, pars = c("a", "G"),
#                             control = list(adapt_delta = 0.9),
#                             data = list(J = 1, N = nrow(Y), Y = Y$NCI, sigmaY = sigmaY,
#                                         K = aperm(Karray, c(3,1,2)),
#                                         P = max(Y$popNum), pop = Y$popNum))
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalPopMeanVarModels.Rdata'))
load(file.path("symcapture_save", 'AnimalPopMeanVarModels.Rdata'))
mcmc_combo(as.array(fitsAnimal$NCI ,  pars = c("sigmaP", "sigmaG", "sigmaR")))
broom::tidyMCMC(fitsAnimal$NCI,  pars = c("sigmaP", "h2"), conf.int = T, rhat = T)
```
...
```{r ANCRAnimal, fig.cap="Proportion of Variance Explained (PVE) by ANCR"}
Y <- trees %>%
  mutate(popNum = as.numeric(as.factor(pop))) %>%
  dplyr::select(FID, IID, ANCR, pop, popNum) %>%
  mutate(ANCR = ANCR/sd(ANCR))
fam <- read_delim(file.path(path, "gemma", "Gmax0", "paracou3pop.fam"), delim = " ", 
                  col_names = c("FID", "IID", "PAT", "MAT", "SEX", "PHENOTYPE"))
K <- read_tsv(file.path(path, "gemma", "Gmax0", "paracou3pop.kinship.sXX.txt"), 
              col_names = fam$IID) %>% 
...
#           globuliferaTypeRegina = filter(Y, pop == "globuliferaTypeRegina"))
# model <- stan_model("./symcapture_models/Animal.stan")
# fitsAnimal$ANCR <- lapply(Yl, function(pop)
#   sampling(model, chains = 2, save_warmup = F, include = F, pars = "a",
#            control = list(adapt_delta = 0.99),
#            data = list(N = nrow(pop), Y = pop$ANCR, K = K[pop$IID,pop$IID])))
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalModels.Rdata'))
load(file.path("symcapture_save", 'AnimalModels.Rdata'))
cowplot::plot_grid(plotlist = lapply(fitsAnimal$ANCR, mcmc_combo, pars = c("sigmaG", "sigmaR")), 
                   labels = names(fitsAnimal$ANCR))
lapply(fitsAnimal$ANCR, broom::tidyMCMC,  pars = c("h2"), conf.int = T, rhat = T, estimate.method = "median") %>% 
  bind_rows(.id = "Population")
```
...
```{r ANCRAnimalPopMeanVar, fig.cap="Proportion of Variance Explained (PVE) by ANCR"}
# Karray <- sapply(1:max(Y$popNum), function(p){
#   Kp = K[Y$IID,Y$IID]
#   for(n in 1:nrow(Y)) {
#     if(Y$popNum[n] != p) {
...
# dim(Karray) <- c(nrow(Y), nrow(Y), max(Y$popNum))
# sigmaY <- group_by(Y, popNum) %>%
#   summarise(sigmaY = var(ANCR)) %>%
#   dplyr::select(sigmaY) %>% unlist()
# model <- stan_model("./symcapture_models/AnimalPopMeanVar.stan")
# fitsAnimal$ANCR <- sampling(model, chains = 2, save_warmup = F, include = F, pars = c("a", "G"),
#                             control = list(adapt_delta = 0.9),
#                             data = list(J = 1, N = nrow(Y), Y = Y$ANCR, sigmaY = sigmaY,
#                                         K = aperm(Karray, c(3,1,2)),
#                                         P = max(Y$popNum), pop = Y$popNum))
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalPopMeanVarModels.Rdata'))
load(file.path("symcapture_save", 'AnimalPopMeanVarModels.Rdata'))
mcmc_combo(as.array(fitsAnimal$ANCR ,  pars = c("sigmaP", "sigmaG", "sigmaR")))
broom::tidyMCMC(fitsAnimal$ANCR,  pars = c("sigmaP", "h2"), conf.int = T, rhat = T)
```

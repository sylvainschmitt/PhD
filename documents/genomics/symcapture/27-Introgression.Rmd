...
# fit.pure <- stan_betareg(h ~ log(Gmax) + TWI + NCI + pair, link = "logit", link.phi = "log", chains = 2,
#                data = left_join(hi.index, trees) %>%
#                  mutate_at(c("TWI", "NCI"), scale, center = F) %>%
#                  mutate_at(c("TWI", "NCI"), as.vector) %>% 
#                  mutate(h = 1-h) %>% 
#                  mutate(h = ifelse(h == 0, 10^-6, h)) %>% 
#                  mutate(h = ifelse(h == 1, 1 - 10^-6, h)) %>% 
#                  filter(h > 0.95 | h < 0.05))
# fit.hyb5 <- stan_betareg(h ~ log(Gmax) + TWI + NCI + pair, link = "logit", link.phi = "log", chains = 2,
#                data = left_join(hi.index, trees) %>%
#                  mutate_at(c("TWI", "NCI"), scale, center = F) %>%
#                  mutate_at(c("TWI", "NCI"), as.vector) %>% 
#                  mutate(h = 1-h) %>% 
...
#                  mutate(h = ifelse(h == 1, 1 - 10^-6, h)) %>% 
#                  filter(h < 0.95, h > 0.05))
# fit.hyb10 <- stan_betareg(h ~ log(Gmax) + TWI + NCI + pair, link = "logit", link.phi = "log", chains = 2,
#                data = left_join(hi.index, trees) %>%
#                  mutate_at(c("TWI", "NCI"), scale, center = F) %>%
#                  mutate_at(c("TWI", "NCI"), as.vector) %>% 
#                  mutate(h = 1-h) %>% 

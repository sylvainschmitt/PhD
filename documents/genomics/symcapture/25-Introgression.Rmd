...
> Another demographic process that can lead to idiosyncratic effects at individual loci is hy-bridization and introgression from related taxa [@Hoban2016]
...
6 genes with 4 genic and 38 outliers with qval < 0.05

No evidence for an allele transfer through introgressed or admixed individuals so far. I think it is not the right way to do it.
...
outliers <- data.frame(gene = c("TRINITY_DN795961_c2_g1", "TRINITY_DN796902_c0_g1", "TRINITY_DN800044_c7_g3", 
                                "TRINITY_DN817465_c3_g1", "TRINITY_DN827983_c2_g1", "TRINITY_DN828088_c2_g1")) %>% 
  left_join(bind_rows(read_tsv(file.path(path, "..", "annotation", "snpsFunc.annotation")) %>%
                        mutate(type = "genic"),
                      read_tsv(file.path(path, "..", "annotation", "snpsHitch.annotation")) %>% 
                        mutate(type = "hitchhiker")) %>% 
              mutate(gene = stringr::str_sub(trsc, 1, -4))) %>% 
  filter(snp %in% filter(bayescan, qval < 0.05)$snp)
...
t <- read_delim(file.path(path, "bayescan", "bayescan.snps.paracou.raw"), delim = " ") %>% 
  ...
  left_join(filter(trees, !is.na(AGR)) %>% 
                              mutate(pop.hybrid = fct_reorder(pop.hybrid, dplyr::desc(TWI))) %>%
                              dplyr::select(pop.hybrid, pop, Ind, IID) %>% unique()) %>% 
...
p1 <- group_by(t, SNP) %>% 
  filter(pop.hybrid == "S. sp1 pure") %>% 
  filter(all(Allele != 1))
 
p2 <- group_by(t, SNP) %>% 
  filter(pop.hybrid == "S. globulifera Paracou pure") %>% 
  filter(all(Allele != 1))

ad <- group_by(t, SNP) %>% 
  filter(pop.hybrid == "S. globulifera Paracou introgressed with S. sp1") %>% 
  filter(all(Allele == 1))

snps <- c(unique(p1$SNP[p1$SNP %in% ad$SNP]), unique(p2$SNP[p2$SNP %in% ad$SNP]))

# snps <- c(unique(p1$SNP[p1$SNP %in% p2$SNP]), unique(p1$SNP[p1$SNP %in% ad$SNP]), unique(p2$SNP[p2$SNP %in% ad$SNP]))

p1 <- group_by(t, SNP) %>% 
  filter(pop.hybrid == "S. sp1 pure") %>% 
  filter(all(Allele != 1))
 
p2 <-  filter(t, SNP %in% outliers$snp) %>% 
  group_by(SNP) %>% 
  filter(pop.hybrid == "S. globulifera Regina pure") %>% 
  filter(all(Allele != 1))

ad <- filter(t, SNP %in% outliers$snp) %>% 
  group_by(SNP) %>% 
  filter(pop.hybrid == "S. sp1 introgressed with S. globulifera Regina") %>% 
  filter(any(Allele == 1))

snps <- c(unique(p1$SNP[p1$SNP %in% p2$SNP]), unique(p1$SNP[p1$SNP %in% ad$SNP]), unique(p2$SNP[p2$SNP %in% ad$SNP]))

ggplot(filter(t, SNP %in% snps) %>% 
...
```{r growthOntogenyAdmixedTab}
mdata <- function(data){
  data <- data %>% 
    filter(!is.na(AGR)) %>% 
    mutate(IndNum = as.numeric(as.factor(Ind)), popNum = as.numeric(as.factor(pop.hybrid)))
  list(N = nrow(data),
       I = length(unique(data$IndNum)),
       G = length(unique(data$popNum)),
       AGR = data$AGR,
       DBH = data$DBH,
...
# save(fitGrowth, file = file.path("symcapture_save", 'growthAdmixed.Rdata'))
load(file.path("symcapture_save", 'growthAdmixed.Rdata'))
...
#   kable(caption = "Summary table of the ontogenic growth model including admixed individuals")
```

```{r growthOntegnyAdmixedPosteriors, fig.cap="Ontogenic parameters distribution at the admixture or the individual level between pure-bred and admixed individuals. Individual parameters distribution represent the parameters estimate distribution, whereas population parameters distribution represent the posterior distribution of the parameter.", fig.width=12}
bind_rows(as.data.frame(fitGrowth, pars = c("Gmaxg", "Doptg", "Ksg")) %>% 
            reshape2::melt(id.vars = NULL, variable.name = "parameter") %>% 
            separate(parameter, c("parameter", "popNum"), "[:punct:]", convert = T) %>% 
            mutate(parameter = gsub("g$", "", parameter)) %>% 
            mutate(level = "Population") %>% 
            left_join(filter(trees, !is.na(AGR)) %>% 
                        mutate(pop.hybrid = fct_reorder(pop.hybrid, dplyr::desc(TWI))) %>%
                        mutate(IndNum = as.numeric(as.factor(Ind)), popNum = as.numeric(as.factor(pop.hybrid))) %>% 
                        dplyr::select(popNum, pop.hybrid, pop) %>% unique()),
          broom::tidyMCMC(fitGrowth, pars = c("Gmaxi", "Dopti", "Ksi")) %>% 
            separate(term, c("parameter", "IndNum"), "[:punct:]", convert = T) %>% 
            ...
            left_join(filter(trees, !is.na(AGR)) %>% 
                        mutate(pop.hybrid = fct_reorder(pop.hybrid, dplyr::desc(TWI))) %>%
                        mutate(IndNum = as.numeric(as.factor(Ind)), popNum = as.numeric(as.factor(pop.hybrid))) %>% 
                        dplyr::select(popNum, pop.hybrid, pop, IndNum, Ind) %>% unique())) %>% 
  mutate(pop = recode(pop.hybrid, "S. globulifera Paracou pure" = "S. globulifera Paracou", "S. sp1 pure" = "S. sp1", 
                      "S. globulifera Regina pure" = "S. gobulifera Regina", 
                      "S. sp1 introgressed with S. globulifera Regina" = "S. sp1",  
                      "S. globulifera Paracou introgressed with S. sp1" = "S. globulifera Paracou", 
  ...
  scale_fill_manual("Genepool", values =  c("lightgrey", "#0000EE", "#1E90FF", "#CD2626"), 
                    guide = guide_legend(nrow = 2)) +
  scale_color_manual("Genepool", values =  c("lightgrey", "#0000EE", "#1E90FF", "#CD2626")) +
  facet_grid(level ~ parameter, scales = "free_x") +
  theme(legend.position = "bottom")
```

```{r DoptOutliersAdmixed, fig.cap="Distribution of alleles along individual optiaml diameter $Dopt$ for the significantly associated SNP between pure-bred and admixed individuals."}
read_delim(file.path(path, "gemma", "Dopt", "Dopt.snps.paracou.raw"), delim = " ") %>% 
  dplyr::select(-FID, -PAT, -MAT, -SEX, -PHENOTYPE) %>% 
  reshape2::melt(id.vars = "IID", variable.name = "SNP", value.name = "Allele") %>% 
  na.omit(Allele) %>%
  mutate(Allele = as.factor(Allele)) %>% 
  mutate(SNP = stringr::str_sub(SNP, 1, -3)) %>% 
  left_join(  broom::tidyMCMC(fitGrowth, pars = "Dopti") %>% 
                separate(term, c("parameter", "IndNum"), "[:punct:]", convert = T) %>% 
                mutate(parameter = gsub("i$", "", parameter)) %>% 
                left_join(filter(trees, !is.na(AGR)) %>% 
                            mutate(pop.hybrid = fct_reorder(pop.hybrid, dplyr::desc(TWI))) %>% 
                            mutate(IndNum = as.numeric(as.factor(Ind)), popNum = as.numeric(as.factor(pop.hybrid))) %>% 
                            dplyr::select(pop, pop.hybrid, IID, Ind, IndNum) %>% unique())) %>% 
  mutate(pop = recode(pop.hybrid, "S. globulifera Paracou pure" = "S. globulifera Paracou", "S. sp1 pure" = "S. sp1", 
                      "S. globulifera Regina pure" = "S. gobulifera Regina", 
                      "S. sp1 introgressed with S. globulifera Regina" = "S. sp1",  
                      "S. globulifera Paracou introgressed with S. sp1" = "S. globulifera Paracou", 
                      "Admixed S. globulifera - S. sp1" = "Admixed", "Admixed S. sp1 - S. globulifera Regina" = "Admixed")) %>% 
  mutate(Allele = recode(Allele, "0" = "CC", "1" = "CG", "2" = "GG")) %>% 
  na.omit(pop.hybrid) %>% 
  ggplot(aes(pop.hybrid, estimate, fill = pop, col = pop, label = Ind)) +
  geom_boxplot(alpha = 0.7) +
  coord_flip() +

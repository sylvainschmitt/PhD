...
We designed in *silico* a set of 20,000 80-mer probes sequences for the genome of *Symphonia globulifera* (Supplementary Material S1). 
...
We aligned transcripts on the new filtered genome draft with `BLAT` [@Kent2002] and selected 533 scaffolds without transcript-match, *e.g.* anonymous scaffolds. 
We finally masked repetitive regions with `RepeatMasker` [@Smit2015] and selected 533 1-kbp loci within the 533 scaffolds previously scaffolds.
...
We investigated population structure using `admixture` [@Alexander2011], with 10 repetitions of K genetic groups varying from 1 to 10. We assessed the number of gene-pools with cross validation.
We defined as admixed and further removed from subsequent analyses individuals with a membership to gene-pools below 90%. 
...
We finally used `bayescan` [@Foll2008] to detect fixed SNPs between the different populations using the 70,737 SNPs filtered for quantitative genomics (see genomic data production).
...
We set to null negative values of kinship as they were confounding with population structure, and we further ensured that the matrix was positive-definitivene using `nearPD`  function from the R package `Matrix`.
...
\begin{equation}
   \begin{aligned}
      y_{p,i} \sim \mathcal{logN}(log(u_{i}),\sigma^2_R) \\
      u_{i} \sim \mathcal{MVlogN_N}(log(\mu_{p}),\sigma^2_G.K)  
   \end{aligned}
   (\#eq:AnimalEnv)
\end{equation}

where $u_i$ is breeding value of the individual $i$ and $\sigma^2_R$ the shape parameter of the lognormal. 
Individual breeding values $u_i$ are defined following a multivariate lognormal law $\mathcal{MVlogN}$ of location $log(\mu_p)$, the logarithm of the mean environment of population $p$, and the co-shape matrix defined as the product of the kinship matrix $K$ with estimated individual genotypic variation $\sigma^2_G$. 
We used lognormal and multivariate lognormal distributions because the two environmental variables were positive and skewed [@LIMPERT2001; @Wan1998]. 
To estimate variances on a normal-scale, we log-transformed population fixed effect, genetic additive values and residuals, and we calculated conditional and marginal $R^2$ [@Nakagawa2013]. 
A Bayesian method was used to infer parameters using `stan` language [@Carpenter2017, see supplementary material SX] and `rstan` package [@Team2018] in the R environment [@RCoreTeam2020] using No-U-Turn Sampler [NUTS, @Hoffman2014] algorithm, performing better for estimating genetic parameters and breeding values [@Nishio2019].
...
We investigated effects of ecological and evolutionary processes on individual growth, using populations, kinship, topographic wetness index and neighbor crowding index.
...
where the difference of DBH $DBH_{y=2017,p,i}-DBH_{y=y_0,p,i}$ is defined with a lognormal distribution of location the logarithm of the sum of annual growth rates $AGR$ during the period $y_0-2017$ and of shape $\sigma^2$. 
...
\begin{equation}
   \begin{aligned}
      Gmax_i \sim \mathcal{logN}(log(u_i + \beta_{TWI}.TWI+\beta_{NCI}.NCI), \sigma^2_{R,Gmax}) \\ 
      u_i \sim \mathcal{MVlogN}(log(Gmax_p), \sigma^2_{G,Gmax}.K)
   \end{aligned}
   (\#eq:AnimalGmax)
\end{equation}

where $u_i$ is the breeding value of individual $i$, $\beta_{TWI}$ and $\beta_{NCI}$ are the effects of TWI and NCI on $Gmax_i$, and $\sigma^2_{R,Gmax}$ is the shape of the lognormal distribution.
Individual breeding values $u_i$ are defined following a multivariate lognormal law $\mathcal{MVlogN}$ with location $log(Gmax_p)$, the logarithm of the mean growth potential of population $p$, and the co-shape matrix defined as the product of the kinship matrix $K$ and the genotypic variation $\sigma^2_{G,Gmax}$. 
...
We used lognormal and multivariate lognormal distributions because growth and individual growth potential were positive and skewed [@LIMPERT2001; @Wan1998]. 
To estimate variances on a normal-scale, we log-transformed population fixed effect, genetic additive values, TWI and NCI fixed effects, and residuals with, and we calculated conditional and marginal $R^2$ [@Nakagawa2013].
We used Bayesian inference with No-U-Turn Sampler [NUTS, @Hoffman2014] using `stan` language [@Carpenter2017, see supplementary material SX] and previously introduced tools.
...
But cross validation of admixture analyses for different levels of clustering K advocated for 3 different gene-pools (Fig. \@ref(fig:admixture), see supplementary material SX). 
...
*S. globulifera type Regina* has also bigger leaves than *S. globulifera type Regina* (see supplementary material SX) but we did not collect fruit or flowers. 
Among the three populations, 25 individuals (7%) showed patterns of introgression, 15 between *S. globulifera type Regina* and *S. sp1* and 10 between *S. globulifera type Paracou* and *S. sp1* (confirmed with `introgress` R package [@Gompert2010], see supplementary material SX). 
...
Site frequency spectrum revealed population in expansion with an excess of rare alleles in each population (see supplementary material SX). 
...
Variance partitioning (Fig. \@ref(fig:Epart)) revealed a high role of topographic wetness index ($TWI$) in genetic differentiation at population and genotypic levels, whereas null to very-weak effect of neighbor crowding index ($NCI$). 
Both $TWI$ and $NCI$ were mostly not explained by population and genotype with 41% and 96% of residual variation and corresponding marginal $R^2$ of 5% [CI:1-17] and 57% [CI:35-94], respectively. 
Still, the variability of TWI where individual grow was strongly explained by the population and the genotype with a conditional $R^2$ of 41% [CI:25-57] and 17% [CI:16-30], respectively. 
Genomic scans revealed 15 significant major effect SNP for $TWI$ but no significant enrichment in gene ontology.

### Growth

The growth modelling showed a high variation in individual growth potential with differences at the population level.
*S. globulifera type Regina* had the higher growth potential and *S. sp1* had the smallest growth potential. 
The variation in growth potential between populations corresponded to variation in maximum observed diameter in Paracou.
Both $TWI$ and $NCI$ had a significant and positive effect on individual growth with stronger effect of $TWI$ ($\beta_{TWI} = 0.24$) over $NCI$ ($\beta_{NCI} = 0.09$). 
The total variation of environmental variables was higher than population and genotypic variation together.
$TWI$ and $NCI$ had a conditional $R^2$ of 13% [CI:7-20] and 3% [CI:1-7], respectively (Fig. \@ref(fig:Ppart)).
While population and genotype had a conditional $R^2$ of 0% [CI:0-2] and 7% [CI:1-35], respectively (Fig. \@ref(fig:Ppart)). 
All the effects resulted in a marginal $R^2$ of 25% [CI:16-50] indicating that around three quarter of the variance remained unexplained. 
Genomic scans revealed 3 significant major effect SNP for individual growth potential (2 for *S. globulifera type Regina* and 1 for *S. sp1*) but no significant enrichment in gene ontology.

# Discussion

> Three populations of *Symphonia* adapted to topography and wetness coexist in Paracou and differ in individual growth potential due to ecological and evolutionary forces.

## Three populations of *Symphonia* coexist in sympatry
...
## Populations and individuals of *Symphonia* are adapted to topography
...
## Environment and genetic effects on individual growth potential **(need something better)**

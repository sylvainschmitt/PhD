...
mdata <- function(data) list(N = nrow(data),
                             ...
                             AGR = data$AGR,
                             DBH = data$DBH,
                             TWIi = dplyr::select(treesSub, IndNum, TWI) %>% unique() %>% 
                               mutate(TWI = scale(TWI, center = F)) %>% arrange(IndNum) %>% 
                               dplyr::select(TWI) %>% unlist(),
                             TWIp = dplyr::select(treesSub, popNum, TWI) %>% unique() %>% 
                               mutate(TWI = scale(TWI, center = F)) %>% group_by(popNum) %>% 
                               summarise_all(mean) %>% arrange(popNum) %>% 
                               dplyr::select(TWI) %>% unlist(),
...
mcmc_intervals_data(fitGrowthKin, regex_pars = c("sigmaP", "sigmaG", "sigmaR")) %>%
  ...
  theme(axis.title.x = element_blank(), axis.title.y = element_blank(),
        axis.text.x = element_blank(), axis.line.x = element_blank(), axis.ticks.x = element_blank()) +
  scale_fill_discrete(expression(sigma^2)) + 
  ggsave(filename = "symcapture_manuscripts/figures/Fig4.png")
```

...
broom::tidyMCMC(fitGrowth, pars = c("theta", "sigmaP", "sigmaR", "sigma", "lp__"), 
  ...
  dplyr::select(ontogeny, parameter, population, estimate, std.error, rhat, ess) %>% 
  kable(caption = "Summary table of the ontogenic growth model",
        col.names = c("Ontogenetic parameter", "Parameter", "Population", "Estimate", "$\\sigma$", "$\\hat{R}$", "$N_{eff}$"))
```

```{r}
mcmc_trace(as.array(fitGrowth, 
                    pars = c("theta", "sigmaP", "sigmaR", "sigma", "lp__")),
           np = nuts_params(fitGrowth))
as.data.frame(fitGrowth, pars = c("sigmaR", "sigma", "lp__")) %>%
  dplyr::rename(sigmaR = "sigmaR[1]") %>% 

...
$$DBH_{y=today,p,i}  - DBH_{y=y0,p,i} \sim \mathcal{logN} ( log(\sum _{y=y_0} ^{y=today} \theta1_i . exp(-\frac12.[\frac{log(\frac{DBH_{y,p,i}}{\theta2_p})}{\theta3_p}]^2)), \sigma_1) \\  \theta1_i \sim \mathcal{logN}(log(a1_i+ \beta_1.TWI+\beta_2.NCI), \sigma_2) \\ \theta2_p \sim \mathcal{logN}(log(\theta2), \sigma_2) \\ \theta3_p  \sim \mathcal{logN}(log(\theta3) ,\sigma_4) \\ a1_i \sim \mathcal{MVlogN}(log(\theta1_p), \sigma_5.K)$$
We fitted the equivalent model with following priors:

$$DBH_{y=today,p,i}  - DBH_{y=y0,p,i} \sim \mathcal{logN} ( log(\sum _{y=y_0} ^{y=today} \theta1_i . exp(-\frac12.[\frac{log(\frac{DBH_{y,p,i}}{\theta2_p})}{\theta3_p}]^2)), \sigma_1) \\  \theta1_i = e^{log(a1_i + \beta_1.TWI + \beta_2.NCI) + \sigma_2.\epsilon1_i} \\ \theta2_p  = e^{log(\theta2) + \sigma_3.\epsilon2_i} \\ \theta3_p  = e^{log(\theta3) + \sigma_4.\epsilon3_i} \\ \\ a1_i = e^{log(\theta1_p) + \sigma_5.A.\epsilon4_i} \\ (\epsilon1, \epsilon2, \epsilon3, \epsilon4) \sim \mathcal{N}(0,1) \\ (\theta1_p, \theta2, \theta3) \sim \mathcal{logN}(0,1) \\ (\beta_1, \beta_2) \sim \mathcal{N}(0,1) \\ (\sigma_1, \sigma_2, \sigma_3, \sigma_4, \sigma_5) \sim \mathcal{N}(0,1) \\ V_P = Var(\mu_p) \\V_G=\sigma_5^2\\V_R=\sigma_2^2$$

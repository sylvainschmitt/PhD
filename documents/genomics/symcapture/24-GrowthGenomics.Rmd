...
$$log(AGR_{t,g,i|g}+1) \sim \mathcal N (Gmax_i.exp(-\frac12.[\frac{log(\frac{DBH_{t,g,i}}{Dopt_i})}{Ks_i}]^2)+u, \sigma_R)$$
$$u \sim \mathcal{MVN}(0,\sigma_gK)$$
$$\begin{bmatrix} Gmax_i \\ Dopt_i \\ Ks_i \end{bmatrix} \sim \mathcal N ^3(\begin{bmatrix} Gmax_g \\ Dopt_g \\ Ks_g \end{bmatrix},\begin{bmatrix} \sigma_{Gmax_i} \\ \sigma_{Dopt_i} \\ \sigma_{Ks_i} \end{bmatrix} )$$
$$\begin{bmatrix} Gmax_g \\ Dopt_g \\ Ks_g \end{bmatrix} \sim \mathcal N ^3(\begin{bmatrix} Gmax \\ Dopt \\ Ks \end{bmatrix},\begin{bmatrix} \sigma_{Gmax_g} \\ \sigma_{Dopt_g} \\ \sigma_{Ks_g} \end{bmatrix} )$$
...
```{r GmaxAnimalPopMean, fig.cap="Proportion of Variance Explained (PVE) by genetic in individual growth potential $Gmax$."}
# model <- stan_model("./symcapture_models/AnimalPopMean.stan")
# fitsAnimal$Gmax <- sampling(model, chains = 2, save_warmup = F, include = F, pars = "a",
#                             control = list(adapt_delta = 0.99),
#                             data = list(J = 1, N = nrow(Y), Y = Y$Gmax, K = K[Y$IID,Y$IID],
#                                         P = max(Y$popNum), pop = Y$popNum))
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalPopMeanModels.Rdata'))
load(file.path("symcapture_save", 'AnimalPopMeanModels.Rdata'))
...
$$y_{p,i} \sim \mathcal{N}(\mu + u_i, \sigma_R)$$
$$u \sim \mathcal{N}_N(0, \sum _p \sigma_{G_p} K_p)$$

```{r GmaxAnimalPopVar, fig.cap="Proportion of Variance Explained (PVE) by genetic in individual growth potential $Gmax$."}
...
# model <- stan_model("./symcapture_models/AnimalPopVar.stan")
...
#                                         P = max(Y$popNum), pop = Y$popNum))
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalPopVarModels.Rdata'))
load(file.path("symcapture_save", 'AnimalPopVarModels.Rdata'))
mcmc_combo(as.array(fitsAnimal$Gmax ,  pars = c("sigmaG", "sigmaR")))
broom::tidyMCMC(fitsAnimal$Gmax,  pars = c("h2"), conf.int = T, rhat = T)
```

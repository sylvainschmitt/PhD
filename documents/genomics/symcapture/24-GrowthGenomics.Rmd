...
K <- read_tsv(file.path(path, "gemma", "Gmax", "paracou3pop.kinship.sXX.txt"), 
  ...
  as.matrix()
# A1 <- K - min(K) # positive K
# A2 <- K/max(K) # standardised K
# A3 <- (K - min(K))/max(K - min(K)) # postive and standardised K
# diag(A) <- diag(A) - min(eigen(A)$values)
checkCovMat <- function(M){ 
  # quick function to check your covariate matrix before running the animal model
  cat("Matrix have dimensions:", dim(M), "\n")
  cat("Matrix is symmetric:", isSymmetric(M), "\n")
  cat("Matrix is positive definite:", matrixcalc::is.positive.definite(M), "\n")
  if(!matrixcalc::is.positive.definite(M))
    cat("Matrix negative eigne values are:", eigen(M)$values[eigen(M)$values < 0], "\n")
  hist(M) # check K distribution
}
# model <- stan_model(paste0("./symcapture_models/Animal.stan"))
# fitsAnimal$Gmax <- sampling(model, chains = 2, data = list(J = 1, N = nrow(Y), Y = Y$Gmax, K = K[Y$IID,Y$IID]), save_warmup = F)
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalModels.Rdata'))
load(file.path("symcapture_save", 'AnimalModels.Rdata'))
g.rel <- cbind(reshape2::melt(as.matrix(dist(Y)), id.vars = NULL, value.name = "Gmax") %>% 
                 dplyr::select(Gmax), 
               reshape2::melt(K[Y$IID,Y$IID], id.vars = NULL, value.name = "Kinship")) %>% 
  ggplot(aes(Kinship, Gmax)) + geom_point(alpha = 0.1)
g.sigma <- mcmc_combo(as.array(fitsAnimal$Gmax ,  pars = c("sigmaG", "sigmaE")))
# mcmc_pairs(as.array(fitsAnimal$Gmax,  pars = c("sigma_G", "sigma_E")))
g.breed <- mcmc_intervals_data(as.array(fitsAnimal$Gmax ,  pars = "u_tilde")) %>% 
            mutate(IID = Y$IID) %>% 
...
Individual optimal diameter is mainly explained by genetic variation ($\sigma_G=0.78\pm0.12$ computed with homemade bayesian animal model and confirmed with both `GEMMA` frequentist LMM and bayesian BSLMM, Fig. \@ref(fig:DoptPVE)). This is unsurprisingly similar to $Gmax$ as the two are 10% correlated. Thus $Dopt$ is also mainly an ontogenetic factor because it controls the shape of the growth response to ontogeny and its controlled by genetics ! Individual optimal diameter was significantly associated to four major effect SNPs in the genus (Fig. \@ref(fig:GmaxAssocMLM) and \@ref(fig:DoptOutliersHist), **I don't discuss *S. globulifera type Regina* because i suspect false positive of minor alleles in reduced population**). The polygenic model (BSLMM) didn't converge for the whole genus and the genepools alone (Fig. \@ref(fig:BSLMMGmaxConvergence)).

### Proportion of Variance Explained

```{r DoptPVE, fig.cap="Proportion of Variance Explained (PVE) by genetic in individual optimal diameter $Dopt$."}
load(file.path("symcapture_save", 'GrowthEnvModels.Rdata'))
Y <- broom::tidyMCMC(fitsEnv$growthEnv2, "Dopti", droppars = NULL, rhat = F) %>%
              separate(term, c("term", "IndNum"), "([[:punct:]])", convert = T, extra = "drop") %>%
  dplyr::rename(Dopt = estimate) %>%
  left_join(trees) %>%
  dplyr::select(IID, Dopt) %>%
  unique() %>%
  mutate(Dopt = RNOmni::rankNorm(Dopt)) %>% 
  arrange(IID)
# model <- stan_model(paste0("./symcapture_models/Animal.stan"))
# fitsAnimal$Dopt <- sampling(model, chains = 2, data = list(J = 1, N = nrow(Y), Y = Y$Dopt, K = K[Y$IID,Y$IID]), save_warmup = F)
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalModels.Rdata'))
load(file.path("symcapture_save", 'AnimalModels.Rdata'))
g.rel <- cbind(reshape2::melt(as.matrix(dist(Y)), id.vars = NULL, value.name = "Dopt") %>% 
                 dplyr::select(Dopt), 
               reshape2::melt(K[Y$IID,Y$IID], id.vars = NULL, value.name = "Kinship")) %>% 
  ggplot(aes(Kinship, Dopt)) + geom_point(alpha = 0.1)
g.sigma <- mcmc_combo(as.array(fitsAnimal$Dopt ,  pars = c("sigmaG", "sigmaE")))
# mcmc_pairs(as.array(fitsAnimal$Dopt,  pars = c("sigma_G", "sigma_E")))
g.breed <- mcmc_intervals_data(as.array(fitsAnimal$Dopt ,  pars = "u_tilde")) %>% 
            mutate(IID = Y$IID) %>% 
            left_join(dplyr::select(trees, IID, pop)) %>% 
  ggplot(aes(x = datatools::reorder_within(IID, m, pop), 
             xend = datatools::reorder_within(IID, m, pop)),
...
fam <- read_delim(file.path(path, "gemma", "Gmax", "paracou3pop.fam"), delim = " ", 
                  col_names = c("FID", "IID", "PAT", "MAT", "SEX", "PHENOTYPE"))
# model <- stan_model(paste0("./symcapture_models/Animal.stan"))
# fitsAnimal$Ks <- sampling(model, chains = 2, data = list(J = 1, N = nrow(Y), Y = Y$Ks, K = K[Y$IID,Y$IID]), save_warmup = F)
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalModels.Rdata'))
load(file.path("symcapture_save", 'AnimalModels.Rdata'))
g.rel <- cbind(reshape2::melt(as.matrix(dist(Y)), id.vars = NULL, value.name = "Ks") %>% 
                 dplyr::select(Ks), 
               reshape2::melt(K[Y$IID,Y$IID], id.vars = NULL, value.name = "Kinship")) %>% 
  ggplot(aes(Kinship, Ks)) + geom_point(alpha = 0.1)
g.sigma <- mcmc_combo(as.array(fitsAnimal$Ks ,  pars = c("sigmaG", "sigmaE")))
# mcmc_pairs(as.array(fitsAnimal$Ks,  pars = c("sigma_G", "sigma_E")))
g.breed <- mcmc_intervals_data(as.array(fitsAnimal$Ks ,  pars = "u_tilde")) %>% 
            mutate(IID = Y$IID) %>% 
            left_join(dplyr::select(trees, IID, pop)) %>% 
  ggplot(aes(x = datatools::reorder_within(IID, m, pop), 
             xend = datatools::reorder_within(IID, m, pop)),
...
Individual vitality index is partly explained by genetic variation with a great incertitude ($\sigma_G=0.25\pm0.18$ computed with homemade bayesian animal model and confirmed with both `GEMMA` frequentist LMM and bayesian BSLMM, Fig. \@ref(fig:VitalityPVE)). Individual vitality index wasn't significantly associated to any major effect SNPs in the whole genus or any of the genepools (Fig. \@ref(fig:VitalityAssocMLM)). The polygenic model (BSLMM) didn't converge for the whole genus and the genepools alone (Fig. \@ref(fig:BSLMMVitalityConvergence)).
...
```{r VitalityPVE, fig.cap="Proportion of Variance Explained (PVE) by genetic in individual vitality index."}
load(file.path("symcapture_save", 'GrowthEnvModels.Rdata'))
Y <- left_join(trees, pars1) %>% 
  mutate(Gpred = Gmax_m*exp(-0.5*(log(DBH/Dopt_m)/Ks_m)^2)*exp(-beta1_m*TWI)*exp(-beta2_m*AUC)*exp(-beta3_m*logNCI)*exp(-beta4_m*logANCR)) %>% 
  mutate(Vitality = log(AGR+1)-Gpred) %>% 
  group_by(FID, IID) %>% 
  arrange(CensusYear) %>% 
  summarise(Vitality = sum(Vitality)/(last(CensusYear) - first(CensusYear))) %>% 
  mutate(Vitality = RNOmni::rankNorm(Vitality)) %>%
  dplyr::select(Vitality, IID) %>%
  arrange(IID)
fam <- read_delim(file.path(path, "gemma", "Gmax", "paracou3pop.fam"), delim = " ", 
                  col_names = c("FID", "IID", "PAT", "MAT", "SEX", "PHENOTYPE"))
# model <- stan_model(paste0("./symcapture_models/Animal.stan"))
# fitsAnimal$Vitality <- sampling(model, chains = 2, data = list(J = 1, N = nrow(Y), Y = Y$Vitality, K = K[Y$IID,Y$IID]), save_warmup = F)
# save(fitsAnimal, file = file.path("symcapture_save", 'AnimalModels.Rdata'))
load(file.path("symcapture_save", 'AnimalModels.Rdata'))
g.rel <- cbind(reshape2::melt(as.matrix(dist(Y)), id.vars = NULL, value.name = "Vitality") %>% 
                 dplyr::select(Vitality), 
               reshape2::melt(K[Y$IID,Y$IID], id.vars = NULL, value.name = "Kinship")) %>% 
  ggplot(aes(Kinship, Vitality)) + geom_point(alpha = 0.1)
g.sigma <- mcmc_combo(as.array(fitsAnimal$Vitality ,  pars = c("sigmaG", "sigmaE")))
# mcmc_pairs(as.array(fitsAnimal$Vitality,  pars = c("sigma_G", "sigma_E")))
g.breed <- mcmc_intervals_data(as.array(fitsAnimal$Vitality ,  pars = "u_tilde")) %>% 
            mutate(IID = Y$IID) %>% 
            left_join(dplyr::select(trees, IID, pop)) %>% 
  ggplot(aes(x = datatools::reorder_within(IID, m, pop), 
             xend = datatools::reorder_within(IID, m, pop)),

...
# length(unique(trees$IID)) # should be 372
...
```{r mdata}
trees <- trees %>% 
  # filter(Plot == 16) %>% # for test purposes
...
treesSub <- filter(trees, Plot == 16) %>% # for test purposes
  mutate(IndNum = as.numeric(as.factor(Ind)), popNum = as.numeric(as.factor(pop)))
fitsAnimal <- list()
mdata <- list(N = nrow(trees),
              I = length(unique(trees$Ind)),
              G = length(unique(trees$pop)),
              AGR = trees$AGR,
              DBH = trees$DBH,
              TWI = as.vector(scale(trees$TWI, center = F)),
              AREW = as.vector(scale(trees$AREW, center = F)),
              NCI = as.vector(scale(trees$NCI, center = F)),
              ind = trees$IndNum,
              gp = trees$popNum,
              indingp = unique(arrange(trees, IndNum)[c("IndNum", "popNum")])$popNum)
mdataSub <- list(N = nrow(treesSub),
              I = length(unique(treesSub$Ind)),
              G = length(unique(treesSub$pop)),
              AGR = treesSub$AGR,
              DBH = treesSub$DBH,
              TWI = as.vector(scale(treesSub$TWI, center = F)),
              AREW = as.vector(scale(treesSub$AREW, center = F)),
              NCI = as.vector(scale(treesSub$NCI, center = F)),
              ind = treesSub$IndNum,
              gp = treesSub$popNum,
              indingp = unique(arrange(treesSub, IndNum)[c("IndNum", "popNum")])$popNum)
```
...
data.frame(DBH = rep(1:100, mdata$I), 
  ...
  ggplot(aes(x = DBH)) +
  # geom_ribbon(aes(ymin = Gpred5, ymax = Gpred95), alpha = 0.5, fill = "lightgrey") +
  # geom_ribbon(aes(ymin = Gpred5g, ymax = Gpred95g, group = pop), alpha = 0.5, fill = "lightgrey") +
...
read_tsv(file.path(path, "gemma", "Gmax", paste0("paracou3pop",".polygenic.Gmax.param.txt"))) %>% 
...
for pop in "${pops[@]}" ; do  gemma -bfile $pop -k -bslmm 1 -o $pop.BSLMM.Dopt ; done

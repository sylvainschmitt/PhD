...
```{r aNCInGmax, fig.cap="Multiplicative genetic values for NCI vs Gmax"}
fitNCI <- list()
...
lapply(c(fitNCI, fitGmaxGeno), function(fit)
  ...
  dplyr::select(IndNum, aNCI, aGmax) %>% 
  left_join(trees) %>% 
  ggplot(aes(aNCI, aGmax, col = pop, size = DBHtoday)) +
  ...
  ylab("Multiplicative genetic value for Gmax") +
  geom_smooth(method = "lm", aes(gruop = NA))
```

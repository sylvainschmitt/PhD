...
K %>% 
  ...
  reshape2::melt("IID1", variable.name = "IID2", value.name = "raw.kinship") %>% 
  left_join(dplyr::select(trees, IID, pop) %>% 
              rename(IID1 = IID, pop1 = pop)) %>% 
    left_join(dplyr::select(trees, IID, pop) %>% 
              rename(IID2 = IID, pop2 = pop)) %>% 
  filter(pop1 == pop2) %>% 
  mutate(positive.kinship = ifelse(raw.kinship < 0, 0, raw.kinship)) %>% 
  mutate(scaled.kinship = positive.kinship*2) %>% 
  # dplyr::select(pop1, raw.kinship, positive.kinship, scaled.kinship) %>% 
  dplyr::select(pop1, positive.kinship) %>% 
  reshape2::melt("pop1") %>% 
  # ggplot(aes(value)) +
  # geom_histogram() +
  # facet_grid(pop1 ~ variable, scales = "free")
  group_by(pop1, variable) %>%
...
fitTWI <- sflist2stanfit(fitTWI)
t2 <- lapply(c(fitNCI, fitTWI, fitGmaxGenoEnv), function(fit)
  as.data.frame(fit, "alog") %>%
  summarise_all(median) %>%
  reshape2::melt(NULL)) %>%
  bind_rows(.id = "fit") %>% 
...
```{r spcorrelo2, eval=F}
cor2 <- lapply(c(TWI = "TWI", NCI = "NCI", Gmax = "Gmax", 
                gTWI = "aTWI", gNCI = "aNCI", gGmax = "aGmax"), function(var) 
  pgirmess::correlog(data.frame(t2$Xutm, t2$Yutm), t2[,var], method = "Moran", nbclass = 100) %>% 
    as.data.frame()) %>% 
  bind_rows(.id = "variable")
cor2 %>% 
  filter(dist.class < 10^3) %>% 
...
```{r outlines, eval=F}
save(fitGmaxGeno, fitGmaxGenoEnv, fitNCI, t, t2, cor, cor2, file = "symcapture_save/outlines.Rdata")
```

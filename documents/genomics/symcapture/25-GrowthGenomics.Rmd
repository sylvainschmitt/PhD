...
```{r gmaxParTrace, fig.cap="Traceplot of model parameters.", eval=F}
...
```{r gmaxParPairs, message=FALSE, fig.cap="Pairs of model parameters.", eval=F}
pairs_stan(1, fitGmax, c("sigmaR", "sigma"))
```

```{r gmaxParEnergy, fig.cap="Energy of the model.", eval=F}
mcmc_nuts_energy(nuts_params(fitGmax))
```

```{r gmaxParPred, fig.cap="Species predicted growth curve.", eval=F}
...
```{r gmaxR2, fig.cap="$R^2$ for Gmax.", eval=F}
mcmc_intervals(fitGmax, "R2m")
```

```{r GmaxVarPart, fig.cap="Genetic variance partitionning for Gmax.", eval=F}
...
```{r gmaxgenoParTrace, fig.cap="Traceplot of model parameters.", eval=F}
...
```{r gmaxgenoParPairs, message=FALSE, fig.cap="Pairs of model parameters.", eval=F}
pairs_stan(1, fitGmaxGeno, c("sigma"))
```

```{r gmaxgenoParEnergy, fig.cap="Energy of the model.", eval=F}
mcmc_nuts_energy(nuts_params(fitGmaxGeno))
```

```{r gmaxgeno1ParPred, fig.cap="Species predicted growth curve.", eval=F}
ggplot(data.frame(DBH = 0:200, AGR = 2), aes(x = DBH, y = AGR)) +
  geom_vline(xintercept = mdata$DBH0, alpha = 0.1) +
  stat_function(aes(col = "S. globulifera Paracou"), fun = function(.x) 
...
```{r gmaxgenoR2, fig.cap="$R^2$ for Gmax.", eval=F}
mcmc_intervals(fitGmaxGeno, c("R2m", "R2c"))
```

```{r GmaxGenoVarPart, fig.cap="Genetic variance partitionning for Gmax.", eval=F}
...
```{r gmaxgenoenvTrace, fig.cap="Traceplot of model parameters.", eval=F}
...
```{r gmaxgenoenvPairs, message=FALSE, fig.cap="Pairs of model parameters.", eval=F}
pairs_stan(1, fitGmaxGenoEnv, c("sigmaR", "sigma"))
```

```{r gmaxgenoenvEnergy, fig.cap="Energy of the model.", eval=F}
mcmc_nuts_energy(nuts_params(fitGmaxGenoEnv))
```

```{r gmaxgenoenvPred, fig.cap="Species predicted growth curve.", eval=F}
ggplot(data.frame(DBH = 0:200, AGR = 2), aes(x = DBH, y = AGR)) +
  geom_vline(xintercept = mdata$DBH0, alpha = 0.1) +
  stat_function(aes(col = "S. globulifera Paracou"), fun = function(.x) 
...
```{r gmaxgenoenvVar, fig.cap="Genetic variance partitionning for ontogenetic parameters.", eval=F}

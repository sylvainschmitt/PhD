...
$$Trait_{t,s,i} \sim \mathcal{logN}(log((\alpha_{t,i}+\delta_P).\frac{DBH_i}{{\beta_{DBH}}_{t,s} + DBH_i}), \sigma_3) \\ \alpha_{t,i} \sim \mathcal{MVlogN}(log(\alpha_{t,s}), \sigma_2.K) \\ \delta_P\sim\mathcal {logN}(0,\sigma_3)$$

We fitted the equivalent model with following priors:

$$Trait_{t,s,i} \sim \mathcal{logN}(log((e^{log(\alpha_{t,s}) +\sigma_2.A.\epsilon1_i+\sigma_3.\epsilon2_p}).\frac{DBH_i}{{\beta_{DBH}}_{t,s} + DBH_i}), \sigma_3) \\ (\epsilon1, \epsilon2) \sim \mathcal N(0,1) \\ \beta_{DBH} \sim \mathcal{logN}(0,1) \\ (\sigma_1,\sigma_2,\sigma_3) \sim \mathcal{N}(0,1) \\ V_s = Var(\alpha_s) \\V_G=\sigma_2^2\\V_P=\sigma_3^2\\V_R=\sigma_1^2$$

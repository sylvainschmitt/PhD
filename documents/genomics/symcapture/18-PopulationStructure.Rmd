...
```{r popgenome, include=FALSE}
library(PopGenome)
genome <- readData(file.path(path, "..", "variantCalling", "paracouOut"), format = "VCF")
fam <- read_tsv(file.path(path, "..", "variantCalling", "globulifera",
                          "symcapture.all.biallelic.snp.filtered.nonmissing.globulifera.fam"),
         col_names = c("FID", "IID", "FIID", "MIID", "sex", "phenotype")) %>% 
  mutate(Ind = gsub(".g.vcf", "", IID))
...
```{r popgenomeG, fig.cap="Introgression within the *Symphonia globulifera* species complex."}
data.frame(region.names = genome@region.names,
           df = as.numeric(genome@df)) %>% 
  mutate(Z = (df-mean(genome@df))/genome@df.SE) %>% 
  mutate(P = 2*(pnorm(-abs(Z)))) %>% 
  mutate(adjP = p.adjust(1-P, "BH")) %>% 
  group_by(region.names) %>% 
  mutate(pos = mean(as.numeric(strsplit(as.character(region.names)," ")[[1]][c(1,3)]))) %>% 
  ungroup() %>% 
  ggplot(aes(pos, df)) +
  geom_hline(yintercept = 0, col = "lightgrey", type = "dashed") +
  geom_line() + 
  geom_point(aes(col = (1-adjP)<0.01, alpha = (1-adjP)<0.01)) +
  geom_label(aes(x, y, label = label),
             data = data.frame(x = 3000, y = c(-0.3, 0.30),
                               label = c("Ssp1 <-> SgParacou", "SgRegina<->SgParacou"))) +
...
<!-- data("LociDataSim1") -->
<!-- help("LociDataSim1") -->
<!-- count.matrix <- prepare.data(admix.gen = AdmixDataSim1, loci.data = LociDataSim1, parental1 = "P1", parental2 = "P2", -->
<!--                              pop.id = F, ind.id = F, fixed = T) -->
<!-- hi.index <- est.h(introgress.data = count.matrix, loci.data = LociDataSim1, fixed = T, -->
<!--                 p1.allele = "P1", p2.allele = "P2") -->
<!-- mk.image(introgress.data = count.matrix, loci.data = LociDataSim1, marker.order = NULL, hi.index = hi.index, -->
<!--          ylab = "Individuals", xlab.h = "population 2 ancestry", pdf = F) -->
<!-- gen.out <- genomic.clines(introgress.data = count.matrix, hi.index = hi.index, -->
<!--                           loci.data = LociDataSim1, sig.test = T, method = "permutation") -->
...
individuals[which.min(abs(pictures[6,]$datetime - individuals$datetime)),]
individuals

cbind(pictures,
  sapply(pictures$datetime, function(x) 
  individuals[which.min(abs(x - individuals$datetime)),], simplify = F) %>% 
      bind_rows())

sapply(1:nrow(pictures), function(x)
  cbind(pictures[x,], individuals[which.min(abs(pictures[x,]$datetime - individuals$datetime)),]))
 x <- nrow(pictures)
 cbind(pictures[x,], individuals[which.min(abs(pictures[x,]$datetime - individuals$datetime)),])

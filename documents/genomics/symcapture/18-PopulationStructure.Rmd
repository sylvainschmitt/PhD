...
library(introgress)
library(magrittr)
paracou <- data.table::fread("symcapture.all.biallelic.snp.filtered.nonmissing.paracou.recode.strct_in", skip = 2)
loci <- readr::read_tsv("symcapture.all.biallelic.snp.filtered.nonmissing.paracou.bim",
  ...
  as.matrix()
paracou <- as.matrix(t(paracou[-1]))
count.matrix <- prepare.data(admix.gen = paracou, loci.data = loci, parental1 = "1", parental2 = "2",
                             pop.id = F, ind.id = F, fixed = T)
hi.index <- est.h(introgress.data = count.matrix, loci.data = loci, fixed = T,
                p1.allele = "1", p2.allele = "2")
mk.image(introgress.data = count.matrix, loci.data = loci, marker.order = NULL, hi.index = hi.index,
         ylab = "Individuals", xlab.h = "population 2 ancestry", pdf = T)
gen.out <- genomic.clines(introgress.data = count.matrix, hi.index = hi.index,
                          loci.data = loci, sig.test = T, method = "permutation")
save(hi.index, gen.out, "introgress.Rdata")
```
...
```{r genepoolsPictures, eval=F}
# ls *.JPG > files.txt
# ~/Tools/Image-ExifTool-11.65/exiftool *.JPG | grep "Date Time UTC" > time.txt
# paste files.txt time.txt > infos.txt
pictures <- read_tsv("../../../data/Symphonia_Paracou/PicturesRenamed/infos.txt",
         col_names = c("file", "datetime")) %>% 
  mutate(datetime = gsub("Date Time UTC                   : ", "", datetime, fixed = T)) %>% 
...
individuals <- googlesheets::gs_title("Measures_Symphonia") %>% 
  ...
  mutate(datetimeInd = paste0(year, ":", month, ":", day, " ", hour, ":", minute, ":00")) %>% 
  dplyr::select(-year, -month, -day, -hour, -minute) %>% 
  mutate(datetimeInd = lubridate::as_datetime(datetimeInd))
trees <- src_sqlite(file.path("/home/sylvain//Documents/BIOGECO/PhD/data/Paracou/",
                              "trees", "Paracou.sqlite")) %>% 
  tbl("Paracou") %>% 
  filter(Genus == "Symphonia") %>% 
  ...
  filter(Plot != 18) %>% 
  collect()
pictures <- cbind(pictures,
  sapply(pictures$datetime, function(x) 
  individuals[which.min(abs(x - individuals$datetimeInd)),], simplify = F) %>% 
      bind_rows()) %>% 
  mutate(Ind = paste0("P", Plot, "_", SubPlot, "_", TreeFieldNum)) %>% 
  group_by(Ind) %>% 
  mutate(filename = paste0(Ind, "_", 1:n(), ".JPG")) %>%
  left_join(trees)
pictures %>% 
  ungroup() %>% 
  mutate(coordCLI = paste0("~/Tools/Image-ExifTool-11.65/exiftool -GPSLongitudeRef=E -GPSLongitude=", Lon,
                      " -GPSLatitudeRef=N -GPSLatitude=", Lat, " ", file)) %>% 
  dplyr::select(coordCLI) %>% 
  write_tsv("../../../data/Symphonia_Paracou/PicturesRenamed/coordinates.sh", col_names = F)
# bash coordinates.sh
# rm *_original
pictures %>% 
  ungroup() %>% 
  mutate(rename = paste("mv", file, filename)) %>% 
  dplyr::select(rename) %>% 
  write_tsv("../../../data/Symphonia_Paracou/PicturesRenamed/rename.sh", col_names = F)
# bash rename.sh
# Beware this is still brutal and lof of pictures doesn't correspond exactly with the time stamp, but it's the best automatic method for the moment
```

...
\begin{equation} 
  y_{p,i} \sim \mathcal{logN}(log(\mu_p) + \sigma_2.A.\epsilon_i, \hat{\sigma_1}) \\ 
  \epsilon_i \sim \mathcal{N}(0,1) \\ 
  ~ \\
  \mu_p \sim \mathcal{logN}(log(1),1) \\  
  \sigma_1 \sim \mathcal N_T(0,1) \\ 
  \hat{\sigma_1} = \sqrt(V_R)
  ~ \\
  V_Y = Var(log(y)) \\
  V_P = Var(log(\mu_p)) \\
  V_G = \sigma_2^2 \\
...
```{r envGenoTab2, fig.cap="Genetic variance of individual growth potential with a lognormal animal model."}
fitEnv <- list(TWI = list(), NCI = list())
for(var in c("TWI", "NCI")){
  for(sim in list.files("symcapture_save/EnvGeno2", 
                        pattern = var, full.names = T)){
    load(sim)
    fitEnv[[var]] <- c(fitEnv[[var]], fit)
  }
...
```{r envGenoTrace2, fig.cap="Traceplot for environmental variables."}
cowplot::plot_grid(plotlist = lapply(fitEnv, mcmc_trace, 
                                     regex_pars = c("Vp", "Vg", "Vr"), 
                                     facet_args = list(nrow = 1)),
                   nrow = 2, labels = names(fitEnv)) 
...
```{r envGenoVarPart2, fig.cap="Genetic variance partitionning for environmental variables."}
lapply(fitEnv, mcmc_intervals_data, regex_pars = c("Vp", "Vg", "Vr")) %>% 
  bind_rows(.id = "variable") %>% 
  mutate(parameter = recode(parameter, "Vp" = "Population", "Vg" = "Genotype", "Vr" = "Residual")) %>% 
  group_by(variable) %>%

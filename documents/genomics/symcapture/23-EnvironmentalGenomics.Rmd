...
with the same parameters as $NCI_i$. $ANCR$ represents the dynamic of biotic asymmetric competition through tree neighborhood over time (derivate of $NCI_i$). Both NCI and ANCR were calculated among heterospecific and conspecific neighbors and with all enighbors confounded. Both correlations and principal component analyses showed non colinear TWI, NCI and ANCR that we selected for further analyses (Fig. \@ref(fig:correlationsEnvGenom)).
...
mdata <- function(data, variable) 
  list(N = nrow(data),
       ...
       K = K[data$IID, data$IID])
dplyr::select(trees, FID, IID, TWI, NCI, ANCR) %>%
...
Most of the variability for topography was explained by the population (58%) and around 30% of the remaining variability was explained by the genotype. On the opposite, most of the variability for competition was explained by the genotype (40-50%) and around 37% of the remaining variability was explained by the population (Tab. \@ref(tab:EnvAnimal) and Fig. \@ref(fig:envKinshipG1) and \@ref(fig:envKinshipG2)). Kinship and population explain more than 80% of the variation from neighboring crowding, so there is a within population adaptation to competition and light access. Variation with topgraphy was more differentiated between population than random genetic variation ($Qst>>Fst$), highlighting a local adptation of *Symphonia* populations to topography.
...
```{r envKinshipG1, fig.cap="Heritability $h^2$, broad heritability $h_p^2$ and trait differentiation $Qst$ for environmental variables."}
lapply(fitEnvKin, mcmc_intervals_data, regex_pars = c("h2", "h2p", "Qst")) %>% 
  bind_rows(.id = "variable") %>% 
  mutate(index = recode(parameter, "h2" = "h^2 == frac(sigma[G]^2,sigma[P]^2+sigma[G]^2+sigma[R]^2)",
                        "h2p" = "h[p]^2 == frac(sigma[P]^2+sigma[G]^2,sigma[P]^2+sigma[G]^2+sigma[R]^2)",
                        "Qst" = "Q[st] == frac(sigma[P]^2,sigma[P]^2+2*sigma[G]^2)")) %>%
  ggplot(aes(x = variable, xend = variable, col = variable, fill = variable)) +
...
lapply(fitEnvKin, mcmc_intervals_data, regex_pars = c("sigmaP", "sigmaG", "sigmaR")) %>% 
  ...
  theme(axis.title.x = element_blank(), axis.title.y = element_blank(),
        axis.text.x = element_blank(), axis.line.x = element_blank(), axis.ticks.x = element_blank()) +
  scale_fill_discrete(expression(sigma^2)) + 
  ggsave(filename = "symcapture_manuscripts/figures/Fig3.png")
```
...
Results are similar to the previous paragraph, they onlyt inform a little bit on within-population genetic variation. Globally the genetic variances are not significantly different exceot for *S. sp1* with an increased genotypic variability with ANCR but decreased with TWI. I'm not sure I will use these results, so they aren't discussed for the moment.
...
# Y <- lapply(c("TWI", "NCI", "ANCR"), function(var){
#  y <- mdata(trees, var) ; y$K <- aperm(Karray, c(3,1,2)) ; return(y)})
# fitEnvPop <- lapply(Y, function(y) sampling(animal, chains = 2, data = y, save_warmup = F,
#            include = F, pars = "a", control = list(adapt_delta = 0.99)))
# names(fitEnvPop) <- c("TWI", "NCI", "ANCR")
# save(fitEnvPop, file = file.path("symcapture_save", 'envpop.Rdata'))
load(file.path("symcapture_save", 'envpop.Rdata'))
lapply(fitEnvPop, broom::tidyMCMC, c("muP", "sigmaP", "sigmaG", "sigmaR"),
       droppars = NULL, rhat = T) %>%
  bind_rows(.id = "Variable") %>%
  separate(term, c("parameter", "population"), "[:punct:]", convert = T) %>%
  mutate(population = recode(population, "1" = "S.globuliferaParacou", "2" = "S.globuliferaRegina", "3" = "S.sp1")) %>%
...
```{r envPopG1, fig.cap="Heritability $h^2$, broad heritability $h_p^2$ and trait differentiation $Qst$ for environmental variables per population."}
lapply(fitEnvPop, mcmc_intervals_data, regex_pars = c("h2", "h2p", "Qst")) %>% 
  bind_rows(.id = "variable") %>% 
  ...
  geom_segment(aes(y = l, yend = h), size = 2, alpha = 0.5) +
  coord_flip() +
  facet_grid(variable ~ index, labeller = label_parsed) +
  xlab("") + ylab("") +
  scale_fill_discrete(guide = "none") +
  scale_color_discrete(guide = "none")
```

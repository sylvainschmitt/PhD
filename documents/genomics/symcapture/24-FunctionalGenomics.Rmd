...
```{r ftGenoTab, fig.cap="Genetic variance of individual growth potential with a normal animal model.", eval=F}
...
```{r ftGenoTrace, fig.cap="Traceplot.", eval=F}
...
```{r ftGenoVar, fig.cap="Genetic variance partitionning.", eval=F}
lapply(fitFT, mcmc_intervals_data, regex_pars = c("Vs", "Vg", "Vp", "Vr")) %>% 
  bind_rows(.id = "variable") %>% 
  mutate(variance = recode_factor(parameter, 
                           "Vs" = "Species", "Vg" = "Genotype", 
                           "Vp" = "Plot", "Vr" = "Residual")) %>% 
  group_by(variable) %>%
...
We used the following model to asses genetic and environmental variances associated to leaf functional traits once we accounted for ontogenetic variation through DBH:

\begin{equation} 
  Trait_{t,p,plot,i} \sim \mathcal{logN}(log(\alpha_{t,p}.a_{t,i}.\delta_{plot}.\beta_1.TWI_i.\beta_2.NCI_i.\frac{DBH_i}{{\beta_{DBH}}_{t,p} + DBH_i}), \sigma_3) \\
  a_{t,i} \sim \mathcal{MVlogN}(log(1), \sigma_2.K) \\
  \delta_{plot}\sim \mathcal {logN}(log(1),\sigma_3)
  (\#eq:ftgenoenv)
\end{equation} 
...
\begin{equation} 
  Trait_{t,p,plot,i} \sim \mathcal{logN}(log(\alpha_{t,p}.\hat{a_{t,i}}.\hat{\delta_{plot}}.\beta_1.TWI_i.\beta_2.NCI_i.\frac{DBH_i}{{\beta_{DBH}}_{t,p} + DBH_i}), \sigma_3) \\
  \hat{a_{t,i}} = e^{\sigma_2.A.\epsilon_{1,i}} \\
  \hat{\delta_{plot}} = e^{\sigma_3.\epsilon_{2,plot}} \\
  \epsilon_{1,i} \sim \mathcal N(0,1) \\
  \epsilon_{2,plot} \sim \mathcal N(0,1) \\
  ~ \\
  \beta_{DBH} \sim \mathcal{logN}(log(1),1) \\
  (\beta_1,\beta_2) \sim \mathcal{N}^2(0,1) \\
  (\sigma_1,\sigma_2,\sigma_3) \sim \mathcal{N}_T^3(0,1) \\
  ~ \\
  V_P = Var(log(\alpha_{t,p})) \\
  V_G = \sigma_2^2 \\
...
```{bash ftGenoEnvCluster, eval=F}
traits=(LMA LDMC LT invLA CC)
for trait in "${traits[@]}" ; do for chain in $(seq 8) ; do echo "module purge ; module load compiler/gcc-7.2.0 ; module load system/R-3.5.3 ; R_LIBS_USER=\" \" Rscript FTGenoEnv.R $chain $trait" ; done ; done > FTGenoEnv.sh
sarray -J FTge -o out/%j.FTge.out -e out/%j.FTge.err -t 48:00:00 --constraint=broadwell --cpus-per-task=1 --mail-type=BEGIN,END,FAIL FTGenoEnv.sh
```

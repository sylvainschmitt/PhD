...
parameters {
  ...
  vector<lower=0>[3] sigma ; // variances
}
model {
  height ~ lognormal(log(mu + sigma[2]*epsilon_b[bloc] + sigma[3]*epsilon_p[provenance] 
                       + beta[1]*age + beta[2]*square(age)
                       + beta[3]*bio5 + beta[4]*bio14 + Q*gamma), sigma[1]) ;
...
generated quantities {
  vector[N] yp = log(mu + sigma[2]*epsilon_b[bloc] + sigma[3]*epsilon_p[provenance] 
                       + beta[1]*age + beta[2]*square(age)

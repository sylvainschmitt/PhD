...
g.het_imiss <- read_delim(file.path(path, "Sequences", "calling", "filtered",
...
g.pca <- read_delim(file.path(path, "Sequences", "calling", "filtered",
...
read_delim(file.path(path, "Sequences", "calling", "filtered",
...
With a maximum of 95% of missing data per individual `--mind 0.2`, a maximum of 15% of missing data per SNP `-geno 0.15`, we obtained **4964** biallelic filtered snps for **435** individuals (3 lost).
...
plink2 --bfile filtered/parvicapture.biallelic.snp.filtered \
  ...
  --remove Outliers.fam \
  --mind 0.95 --geno 0.15 \
  --make-bed --out nonmissing/parvicapture.biallelic.snp.filtered.nonmissing
plink --bfile nonmissing/parvicapture.biallelic.snp.filtered.nonmissing \
  --allow-extra-chr --missing --het --freqx --pca \
  --out nonmissing/parvicapture.biallelic.snp.filtered.nonmissing
```
...
g.pca <- read_delim(file.path(pathCluster, "Sequences", "calling", "nonmissing",
...
read_delim(file.path(pathCluster, "Sequences", "calling", "nonmissing",
                                "parvicapture.biallelic.snp.filtered.nonmissing.lmiss"), delim = " ") %>%
  dplyr::rename_all(funs(gsub(" ", "", .))) %>%
  dplyr::rename(contig = CHR) %>%
  mutate(type = ifelse(grepl("TRINITY", contig), "functional", "neutral")) %>% 
  mutate(F_MISS = as.numeric(F_MISS)) %>%
...
With a maximum of 95% of missing data per individual `--mind 0.95`, a maximum of 15% of missing data per SNP `-geno 0.15` and a minimum allele frequency of 10% `-maf 0.1`, we obtained **4964** biallelic filtered snps for **435** individuals (3 lost).

```{bash hard, eval=F, echo=T}
module load bioinfo/plink_high_contig_20190905
module load bioinfo/plink2_high_contig_20190905
rm -r hard
mkdir hard
plink2 --bfile filtered/parvicapture.biallelic.snp.filtered \
  --allow-extra-chr \
  --mind 0.5 --geno 0.0 \
  --make-bed --out hard/parvicapture.biallelic.snp.filtered.nonmissing
plink --bfile hard/parvicapture.biallelic.snp.filtered.nonmissing \
  --allow-extra-chr --missing --het --freqx --pca \
  --out hard/parvicapture.biallelic.snp.filtered.nonmissing
```

```{r hardIndividuals, fig.cap="Missing data statistics for filtered biallelic SNP after missing data filtering (95% for individuals and 15% for SNPs) per individual."}
g.het_imiss <- read_delim(file.path(pathCluster, "Sequences", "calling", "hard",
                                    "parvicapture.biallelic.snp.filtered.nonmissing.imiss"), delim = " ") %>%
  dplyr::rename_all(funs(gsub(" ", "", .))) %>%
  mutate_at(c("N_MISS", "N_GENO", "F_MISS"), as.numeric) %>%
  mutate(IdGenetic = gsub(".g.vcf", "", IID)) %>%
  mutate(IdGenetic = gsub(" ", "", IdGenetic)) %>%
  ...
  scale_fill_discrete("Species", guide = "none") +
  xlab("Proportion of missing SNPs per individual")
g.pca <- read_delim(file.path(pathCluster, "Sequences", "calling", "hard",
                              "parvicapture.biallelic.snp.filtered.nonmissing.eigenvec"),
           delim = " ", col_names = c("X1", "Sample", paste0("PCA", 1:20))) %>%
  mutate(IdGenetic = gsub(".g.vcf", "", Sample)) %>%
  left_join(individuals) %>%
...
```{r hardSNP, fig.cap="Missing data statistics for filtered biallelic SNP after missing data filtering (95% for individuals and 15% for SNPs) per SNP."}
read_delim(file.path(pathCluster, "Sequences", "calling", "hard",
                                "parvicapture.biallelic.snp.filtered.nonmissing.lmiss"), delim = " ") %>%
  dplyr::rename_all(funs(gsub(" ", "", .))) %>%
  dplyr::rename(contig = CHR) %>%
  mutate(type = ifelse(grepl("TRINITY", contig), "functional", "neutral")) %>% 
...
plink2 --bfile filtered/parvicapture.biallelic.snp.filtered \
  --allow-extra-chr \
  --keep nonoutliers3sp.fam \
  --mind 0.95 --geno 0.15 \
  --make-bed --out 3sp/parvicapture.biallelic.snp.filtered.nonmissing
plink --bfile 3sp/parvicapture.biallelic.snp.filtered.nonmissing \
  --allow-extra-chr --missing --het --freqx --pca \
  --out 3sp/parvicapture.biallelic.snp.filtered.nonmissing
```

```{r hardIndividuals, fig.cap="Missing data statistics for filtered biallelic SNP after missing data filtering (95% for individuals and 15% for SNPs) per individual."}
g.pca <- read_delim(file.path(pathCluster, "Sequences", "calling", "3sp",
                              "parvicapture.biallelic.snp.filtered.nonmissing.eigenvec"),
           delim = " ", col_names = c("X1", "Sample", paste0("PCA", 1:20))) %>%
  mutate(IdGenetic = gsub(".g.vcf", "", Sample)) %>%
  left_join(individuals) %>%

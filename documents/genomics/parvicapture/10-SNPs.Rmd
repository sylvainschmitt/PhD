...
g.pca <- read_delim(file.path(pathCluster, "Sequences", "calling", "neutral",
...
g.pca <- read_delim(file.path(pathCluster, "Sequences", "calling", "functional",
                              "parvicapture.biallelic.snp.filtered.nonmissing.eigenvec"),
           delim = " ", col_names = c("X1", "Sample", paste0("PCA", 1:20))) %>%
  mutate(IdGenetic = gsub(".g.vcf", "", Sample)) %>%
  left_join(individuals) %>%
...
g.pca <- read_delim(file.path(pathCluster, "Sequences", "calling", "1k",
                              "parvicapture.biallelic.snp.filtered.nonmissing.eigenvec"),
           delim = " ", col_names = c("X1", "Sample", paste0("PCA", 1:20))) %>%
  mutate(IdGenetic = gsub(".g.vcf", "", Sample)) %>%
  left_join(individuals) %>%
...
googlesheets::gs_title("Parvicapture") %>% 
  ...
  mutate(FID = 0, IID = paste0(IdGenetic, ".g.vcf"), PAT = 0, MAT = 0, SEX = 0, PHENO = -9) %>% 
  dplyr::select(FID, IID, PAT, MAT, SEX, PHENO) %>% 
  write_tsv(path = file.path(pathCluster, "Sequences", "calling", "Repeated.fam"), col_names = F)
googlesheets::gs_title("Parvicapture") %>% 
  googlesheets::gs_read("Extraction") %>% 
  group_by(IdGenetic) %>% 
  filter(n() < 2) %>% 
  ...
  mutate(FID = 0, IID = paste0(IdGenetic, ".g.vcf"), PAT = 0, MAT = 0, SEX = 0, PHENO = -9) %>% 
  dplyr::select(FID, IID, PAT, MAT, SEX, PHENO) %>% 
  write_tsv(path = file.path(pathCluster, "Sequences", "calling", "Unique.fam"), col_names = F)
```
...
g.pca <- read_delim(file.path(pathCluster, "Sequences", "calling", "repeated",
                              "parvicapture.biallelic.snp.filtered.nonmissing.eigenvec"),
           delim = " ", col_names = c("X1", "Sample", paste0("PCA", 1:20))) %>%
  mutate(IdGenetic = gsub(".g.vcf", "", Sample)) %>%
  left_join(individuals) %>%
...
g.pca <- read_delim(file.path(pathCluster, "Sequences", "calling", "unique",
                              "parvicapture.biallelic.snp.filtered.nonmissing.eigenvec"),
           delim = " ", col_names = c("X1", "Sample", paste0("PCA", 1:20))) %>%
  mutate(IdGenetic = gsub(".g.vcf", "", Sample)) %>%
  left_join(individuals) %>%
...
g.pca <- read_delim(file.path(pathCluster, "Sequences", "calling", "outliers",
                              "parvicapture.biallelic.snp.filtered.nonmissing.eigenvec"),
           delim = " ", col_names = c("X1", "Sample", paste0("PCA", 1:20))) %>%
  mutate(IdGenetic = gsub(".g.vcf", "", Sample)) %>%
  left_join(individuals) %>%
...
g.pca <- read_delim(file.path(pathCluster, "Sequences", "calling", "test",
                              "parvicapture.biallelic.snp.filtered.nonmissing.eigenvec"),
           delim = " ", col_names = c("X1", "Sample", paste0("PCA", 1:20))) %>%
  mutate(IdGenetic = gsub(".g.vcf", "", Sample)) %>%
  left_join(individuals) %>%

...
gSample <- lapply(list.files(file.path(path, "Sequences", "quality"), full.names = T, pattern = "general_stats"), read_tsv) %>% 
  ...
  geom_col() +
  geom_hline(yintercept = 666667, linetype = "dashed", col = "red", size = 1.2) +
  coord_flip() +
  scale_y_log10() +

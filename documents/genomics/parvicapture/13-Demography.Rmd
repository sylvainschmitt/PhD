...
```{r setup_demoghraphy, include=FALSE}
...
individuals <- googlesheets::gs_title("Parvicapture") %>% 
  ...
  unique() %>% 
  na.omit() %>% 
  mutate(IID = paste0(IdGenetic, ".g.vcf"))
```
...
library(gdsfmt)
library(SNPRelate)
snpgdsBED2GDS("../../calling/nonmissing/parvicapture.biallelic.snp.filtered.nonmissing.bed",
              "../../calling/nonmissing/parvicapture.biallelic.snp.filtered.nonmissing.fam",
              "../../calling/nonmissing/parvicapture.biallelic.snp.filtered.nonmissing.bim",
              "parvicapture.biallelic.snp.filtered.nonmissing.gds")
geno <- openfn.gds("parvicapture.biallelic.snp.filtered.nonmissing.gds")
dissMatrix  <-  snpgdsDiss(geno, autosome.only = F, num.thread = 3)
...
phylo <- ape::as.phylo(cutTree$dendrogram)
g <- ggtree(phylo, layout='circular')
g +
  geom_tiplab(data = dplyr::inner_join(g$data, individuals, by = c("label" = "IID")),
              aes(col = Species, angle = angle, label = Species), cex = 1) +
  ggsave("parvicapture_save/phylo.png")
```
...
<!-- g.tree <- lapply(c("neutral", "hitchhiker", "functional"), function(type) -->

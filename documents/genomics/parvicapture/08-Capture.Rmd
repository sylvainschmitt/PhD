...
209 libraries have an estimated concentration below 10 $ng.\mu L ^{-1}$, so in addition to repeats library between an estimated concentration of 4.4 and 8 $ng.\mu L ^{-1}$ have been reamplified with 8 cycles.

```{r reamplification, eval=F, echo=T}
quantit %>% 
  left_join(googlesheets::gs_title("Parvicapture") %>% 
             googlesheets::gs_read("Extraction"),
            by = c("Plate_library" = "Plate_extraction",
...
Repeats have more or less worked with medium concentration of DNA whereas reamplification gave good results (Fig. \@ref(fig:quantit2)).
...
All samples will be pulled by batch of 134 following plate order for the 4 reactions of Pippin (with 1 repeat). We want 100 to 500 We will use $8 ng.\mu L^{-1} \times 20 \mu L = 160 ng$ of DNA of each sample ($8 ng.\mu L^{-1}$ being in the lower concentrations, samples below will be sacrified and considered as 8 because they are repeated), resulting in $160 ng \times 8 = 21.44 \mu g$ of DNA per sample (because we will loose material in purification, we should aim for extra).

...
```{r repeats}
filter(quantit, concentration_est < 4.4) %>% 
  arrange(Plate_library, Position_library) %>% 
  select(Plate_library, Position_library, concentration_est) %>% 
  mutate(Plate_repeat = 6) %>% 
  mutate(Plate_position = as.vector(sapply(1:12, function(x) paste0(LETTERS[1:8], x)))) %>% 
  kable()
```

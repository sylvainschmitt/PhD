...
mdata <- lapply(complexes, function(complex)
  list(N = nrow(data),
       ...
       X = dplyr::select(data,
         ...
         mutate(DBHTWI = DBH*TWI, DBHBA = DBH*BA, 
                DBHBAgens = DBH*BAgenus, DBHBAspecies = DBH*BAspecies) %>% 
         dplyr::select(-DBH) %>% 
...
cat("#### Alert start ####\n\n")
library(RPushbullet)
pbPost("note", "Complexes distribution Reduced", "Sampling start")
...
save(fits, file = "./distribution_save/ComplexesReduced.Rdata")

#### Alert done ####

cat("#### Alert done ####\n\n")
library(RPushbullet)
pbPost("note", "Complexes distribution Reduced", "Sampling done")

...
lapply(names(mdata), function(complex) {
  fit <- sampling(Model, chains = 2, data = mdata[[complex]])  
  save(fit, file = paste0("./distribution_save/species_", complex,  ".Rdata"))
})
...
```{r posterior2, fig.cap="Model parameters posterior.", fig.width=10, fig.height=14}
lapply(lapply(fits, as.array, pars = "gamma"), 
       bayesplot::mcmc_intervals_data) %>% 
  bind_rows(.id = "complex") %>% 
  mutate(descriptor = gsub("([[:alpha:]])", "", parameter)) %>% 
  mutate(descriptor = gsub("([[:punct:]])", "", descriptor)) %>% 
...
preds %>% 
  ...
  left_join(data %>% 
              ...
              group_by(Complex, N_complex, Genus, Species) %>% 
              summarise(N_species = n()) %>% 
              mutate(Yo = N_species / N_complex),
            by = c("complex" = "Complex", "species" = "Species")) %>% 
  do(g = ggplot(data = ., aes(x = value, col = species)) + 
       geom_ribbon(aes(ymin = (Y5)/Yo, ymax = (Y95)/Yo), alpha = 0.2) +
       geom_line(aes(y = (Y)/Yo), lwd = 1.3) +
       # geom_hline(aes(yintercept = RelAbund, col = species), linetype = "dashed") +
       facet_grid(complex ~ variable, scales = "free_x") +
       guides(col = guide_legend(nrow=2)) +
       theme(legend.position = "bottom")) %>% 
...
```{r sensitivity, fig.cap="Model sensitivity to co-variables."}
preds %>% 
  mutate(Delta = sqrt((Yp-Y)^2)) %>% 
  mutate(SpeciesLong = paste(substr(complex, 1, 1), species)) %>% 
  ggplot(aes(x = SpeciesLong, y = Delta, fill = variable)) +
  geom_boxplot() +
  coord_flip() +
  ylab("Delta prediction") +
  xlab("") +
  scale_y_sqrt()
```

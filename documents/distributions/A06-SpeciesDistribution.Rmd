...
mdata <- lapply(mdata, function(data)
  list(N = nrow(data),
       ...
       X = dplyr::select(data, DEM, TWI, Slope, BA, BAgenus) %>%
...
For test purposes, we only used individual trees belonging to the species complex censused in 2015 in the plot 16 for *Symphonia* and in the plot 1 and subplot 1 for *Eschweilera*. But later we will include more plots and subplots, and maybe more census years.

## Environmental variables

To be defined in document [A03-Descriptors](A03-Descriptors.html), for the moment we used only abiotic descriptors represented by elevation (DEM), wetness index (TWI) and slope (Slope) (but we built a generic form of model that can adapt to change in the descriptor set).
...
lapply(fits, function(fit)
  ...
  mutate(descriptor = recode_factor(descriptor, `1` = "DEM", 
                                    `2` = "TWI", `3` = "Slope",
                                    `4` = "BA", `5` = "BAgenus")) %>% 
  ...
  mutate(species = ifelse(complex == "Symphonia",
                          levels(as.factor(filter(data, 
                                                  Complex == "Symphonia", 
                                                  Plot == 16)$Species))[species],
                          levels(as.factor(filter(data, 
                                                  Complex == "Eschweilera clade Parvifolia",
                                                  Plot == 1)$Species))[species])) %>% 
...
lapply(complexes, function(complex)
  lapply(1:length(unique(mdata[[complex]]$Species)), function(sp)
    cbind(dplyr::select(mdata[[complex]], DEM, TWI, Slope, BA, BAgenus),
          complex = complex,
...
lapply(complexes, function(complex)
  ...
  mutate(species = paste0(substr(complex, 1, 1), ". ", species)) %>% 
  dplyr::select(Species, DBH, theta, TWI, DEM, Slope, BA, BAgenus) %>% 
  reshape2::melt(id.vars = c("Species", "DBH", "theta", "TWI", "BA")) %>% 
...
```{r niche2, fig.cap="Niche (HabitatSuitability)."}
lapply(complexes, function(complex)
  lapply(1:length(unique(mdata[[complex]]$Species)), function(sp){
    pars_val <- apply(as.matrix(fits[[complex]], 
                                pars = c(paste0("alpha[", sp, "]"), 
                                         paste0("beta[", sp, ",1]"), 
                                         paste0("beta[", sp, ",2]"), 
                                         paste0("beta[", sp, ",3]"),
                                         paste0("gamma[", sp, ",1]"), 
    ...
    DEM <- seq(-2, 2, length.out = n)
    TWI <- seq(-2, 2, length.out = n)
    for(i in 1:n)
      for(j in 1:n)
        HS[i, j] <- psych::logistic(pars_val[paste0("alpha[", sp, "]")] + 
                                      pars_val[paste0("beta[", sp, ",1]")]*DEM[i] +
                                      pars_val[paste0("gamma[", sp, ",1]")]*DEM[i]^2 +
                                      pars_val[paste0("beta[", sp, ",2]")]*TWI[j] +
                                      pars_val[paste0("gamma[", sp, ",2]")]*TWI[j]^2 +
                                      pars_val[paste0("beta[", sp, ",3]")]*0.5 +
                                      pars_val[paste0("gamma[", sp, ",3]")]*0.5^2)
    reshape2::melt(HS, value.name = "HS") %>% 
         mutate(Var1 = DEM[Var1],
           Var2 = TWI[Var2]) %>%
      rename(DEM = Var1, TWI = Var2) %>% 
      mutate(species = levels(as.factor(mdata[[complex]]$Species))[sp])
  }) %>% 
    bind_rows()) %>% 
  bind_rows() %>% 
  ggplot(aes(x = DEM, y = TWI, z = HS, fill = HS)) +
  geom_raster() +
  scale_fill_distiller("Habitat\nsuitability (%)", palette = "Spectral") +
  facet_wrap(~ species) +
  coord_fixed() +
  theme(legend.position="bottom")
```

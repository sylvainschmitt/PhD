...
mdata <- lapply(mdata, function(data)
  list(N = nrow(data),
       ...
       X = dplyr::select(data, DEM, TWI, Slope) %>%
...
lapply(fits, function(fit)
  ...
  mutate(descriptor = recode_factor(descriptor, `2` = "DEM", 
                                    `1` = "TWI", `3` = "Slope")) %>% 
...
lapply(complexes, function(complex)
  lapply(1:length(unique(mdata[[complex]]$Species)), function(sp)
    cbind(dplyr::select(mdata[[complex]], DEM, TWI, Slope),
          complex = complex,
          species = unique(mdata[[complex]]$Species)[sp],
...
lapply(complexes, function(complex)
  ...
  mutate(species = paste0(substr(complex, 1, 1), ". ", species)) %>% 
  dplyr::select(Species, DBH, theta, TWI, DEM, Slope) %>% 
  ...
  scale_color_gradient2("Habitat\nsuitability", low = scales::muted("blue"), mid = "white",
                        high = scales::muted("red"), midpoint = 0.3) +
...
lapply(complexes, function(complex)
  lapply(1:length(unique(mdata[[complex]]$Species)), function(sp){
    ...
    reshape2::melt(HS, value.name = "HS") %>% 
      ...
      mutate(species = unique(mdata[[complex]]$Species)[sp])
  }) %>% 
    bind_rows()) %>% 
  bind_rows() %>% 

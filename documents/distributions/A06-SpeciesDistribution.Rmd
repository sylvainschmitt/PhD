...
lapply(fits, function(fit)
  ...
  mutate(term = recode_factor(term, alpha = "$\\alpha$", 
                              beta = "$\\beta$", 
                              gamma = "$\\gamma$")) %>%
  mutate(species = levels(as.factor(filter(data, Complex == complex)$Species))[as.numeric(species)]) %>% 
...
mdata <- lapply(complexes, function(complex) filter(data, Complex == complex))
names(mdata) <- complexes
lapply(complexes, function(complex)
  lapply(1:length(unique(mdata[[complex]]$Species)), function(sp)
    cbind(dplyr::select(mdata[[complex]], TWI, BA, BAgenus),
          complex = complex,
...
lapply(complexes, function(complex)
  ...
  mutate(Species = paste0(substr(complex, 1, 1), ". ", Species)) %>% 
  dplyr::select(Species, DBH, theta, TWI, BA, BAgenus) %>% 

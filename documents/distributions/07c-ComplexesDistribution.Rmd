```{r setup_compdistc, include=FALSE}
...
mdata <- lapply(complexes, function(complex) {
  ...
  list(N = nrow(data_complex),
       ...
       X = dplyr::select(data_complex, TWI, NCIother, NCIhetero) %>%
...
save(fits, file = "./distribution_save/complexes2.Rdata")
load("./distribution_save/complexes2.Rdata")
pars <- c("alpha", "beta", "gamma")
```

# Complexes distributions (NCIhetero)
...
```{r PosteriorNPrediction_compdistc, fig.cap="Model parameters posterior and predictions.", eval=F}
...
```{stan Model_compdistc, output.var="Model", echo=T, eval=F, cache = F}
data {
...
```{r mcmc_compdistc, eval=F, fig.cap="Markov chains trace plot after warmup for leaf thickness model."}

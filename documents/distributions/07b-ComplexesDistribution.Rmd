```{r setup_compdistb, include=FALSE}
...
mdata <- lapply(complexes, function(complex) {
  ...
  list(N = nrow(data_complex),
       ...
       X = dplyr::select(data_complex, TWI, NCIother, NCIcon) %>%
...
# save(fits, file = "./distribution_save/complexes1.Rdata")
load("./distribution_save/complexes1.Rdata")
...
preds <- lapply(names(fits), function(complex){
  ...
  values <- lapply(values, function(x){colnames(x) <- c("TWI", "NCIother", "NCIcon") ; return(x)})
...
# Complexes distributions (NCIcon)
...
```{r PosteriorNPrediction_compdistb, fig.cap="Model parameters posterior and predictions."}
...
```{stan Model_compdistb, output.var="Model", echo=T, eval=F, cache = F}
data {
...
```{r mcmc_compdistb, fig.cap="Markov chains trace plot after warmup for leaf thickness model."}

...
Competition <- Competition %>% 
  ...
  summarise(NCI = sum(DBHj^2*exp(-0.25*dij)),
            NCIgenus = sum(DBHj^2*exp(-0.25*dij)*as.numeric(Genus == Genusj)),
            NCIspecies = sum(DBHj^2*exp(-0.25*dij)*as.numeric(Species == Speciesj)))
```
...
```{r}
data %>% 
  dplyr::select(Complex, Species, NCI, NCIgenus, NCIspecies) %>% 
  mutate(NCI = NCI-NCIspecies) %>% 
  reshape2::melt(id.vars = c("Complex", "Species")) %>% 
  ggplot(aes(value, col = Species, fill = Species)) + 
...
Globally, topographic variables derived from the digital elevation model showed high level of co-variation (correlation between 0.57 and 0.9) at the exception of curvature. Regarding biotic descriptors digital canopy model and basal area showed a non negligible correlation of 0.34 and basal area for genus and species were correlated to 0.74.
...
We thus selected three little correlated variables ($r < 0.13$) to represent species biotic and abiotic environment: (i) topographic wetness index (TWI) representing  water accumulation areas, crucial to tropical ecosystems [@ferry2010higher], (ii) basal area (BA) representing competition and (iii) congeneric basal area (BAgenus) representing biotic interactions between individuals from the same genus.

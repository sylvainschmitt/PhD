...
#     mutate(Yp = mean/max(mean)) %>% 
#   summarise(sp = sum(sqrt((1-Yp)^2))/nrow(.)) %>%
#   reshape2::dcast(complex ~ variable)
# 
# preds %>%
#   left_join(group_by(dataComplex, Complex) %>%
#               summarise(Yo = n()/nrow(.)),
...
#   mutate(Yp = mean - Yo) %>%
#   # mutate(Yp = mean/max(mean)) %>% 
#   ggplot(aes(value, Yp, col = complex)) +
#   geom_line() +
...
# cowplot::plot_grid(plotlist = t$g, ncol = 2)
...
#   mutate(Yp = mean/max(mean)) %>% 
#   # mutate(Yp = mean / RelAbund) %>% 
#   group_by(complex, Species, variable) %>% 
#   summarise(sp = sum(sqrt((1-Yp)^2))/nrow(.)) %>%
#   reshape2::dcast(complex + Species ~ variable)
# 
# predSpecies %>% 
#   group_by(complex) %>% 
#   left_join(data %>%
...
#   # mutate(Yp = mean/max(mean)) %>% 
#   mutate(Yp = mean - RelAbund) %>%
#   ggplot(aes(value, Yp, col = Species)) +
#   geom_line() +
#   geom_hline(yintercept = 0, linetype = "dashed", col = "lightgrey") +

...
load("./distribution_save/ComplexesReduced.Rdata")
...
Table \@ref(tab:summary) and figures \@ref(fig:posterior), \@ref(fig:prediciton) will not be commented yet as they represent preliminary results. 

Still, we can note that globally abiotic and biotic environment through TWI and BA, besides non-null, have little or no effect on species complexes distribution. On the other hand, congeneric basal area (BAgenus) have a little to medium effect on species complexes (especially on *E. charatcea* and *Licania*) with a parabolic answer of optimum near to low consgeneric density but non nul, *i.e.* few congeneric neighboors improves habitat suitability.
...
preds <- lapply(complexes, function(complex){
  ...
  # Yp <- do.call("rbind", sapply(1: nrow(alpha), function(n) (arm::invlogit(alpha[n] + beta[n,1]*TWI + beta[n,2]*BA + beta[n,3]*BAgenus + gamma[n,1]*TWI^2 + gamma[n,2]*BA^2 + gamma[n,3]*BAgenus^2)), simplify = F))

...
mdata <- lapply(complexes, function(complex)
  list(N = nrow(mdata),
       K = 3,
       Y = as.numeric(mdata$Complex == complex),
       X = dplyr::select(mdata, DEM, TWI, Slope) %>%
         mutate_all(funs(scale)) %>%
         as.matrix(),
       w = ifelse(mdata$Complex == complex,
                  1/(2*sum(mdata$Complex == complex)),
                  1/(2*sum(mdata$Complex != complex)))))
...
# save(fits, file = "./distribution_save/Complex.Rdata")
load("./distribution_save/Complex.Rdata")
...
lapply(fits, function(fit) 
  ...
  ggplot(aes(x = parameter, xend = parameter, col = descriptor, fill = descriptor)) +
  ...
  coord_flip() +
  facet_wrap(~ complex, scales = "free", nrow = 2) +
  xlab("") +
...
lapply(fits, function(fit)
  cbind(dplyr::select(filter(data, Plot ==1, SubPlot == 1), DEM, TWI, Slope),
...
lapply(fits, function(fit){
  pars_val <- apply(as.matrix(fit, pars = c("alpha", "beta[1]", "beta[2]", "beta[3]",
                                            "gamma[1]", "gamma[2]", "gamma[3]")), 2, mean)
  n <- 100
  HS <- matrix(nrow = n, ncol = n)
  DEM <- seq(-1, 1, length.out = n)
  TWI <- seq(-1, 1, length.out = n)
  for(i in 1:n)
    for(j in 1:n)
      HS[i, j] <- psych::logistic(pars_val["alpha"] + 
                                    pars_val["beta[1]"]*DEM[i] +
                                    pars_val["gamma[1]"]*DEM[i]^2 +
                                    pars_val["beta[2]"]*TWI[j] +
                                    pars_val["gamma[2]"]*TWI[j]^2 +
                                    pars_val["beta[3]"]*0.5 +
                                    pars_val["gamma[3]"]*0.5^2)

---
title: "A09 : Species distribution and growth"
date: '`r Sys.Date()`'
...
data <- trees %>% 
  ...
  filter(Plot %in% 1) %>% # for test purposes
...
$$Vigor =  (AGR - \hat{AGR}) \sim \mathcal{N} (\alpha+\beta .log(suitability),\sigma)  $$
...
paracou <-  src_sqlite(file.path(path, "trees", "Paracou.sqlite")) %>%
  ...
  filter(ddbh > 0) %>% 
  mutate(agr = ddbh/dt) %>% 
  left_join(select(data, idTree, logSuitability))
```
...
```{r posteriorDist}
bayesplot::mcmc_areas(as.array(fit, pars = c("gSuitability")))
```

---
title: "A06: Species distributions with DBH"
date: '`r Sys.Date()`'
...
load("./distribution_save/SpeciesReduced.Rdata")
...
```{r posteriorDBH, fig.cap="Model parameters posterior."}
lapply(fits, function(fit) 
  mcmc_intervals_data(as.array(fit, pars = pars))) %>% 
  bind_rows(.id = "complex") %>% 
  mutate(descriptor = gsub("([[:alpha:]])", "", parameter)) %>% 
  ...
  mutate(species = substr(descriptor, 1, 1)) %>% 
  mutate(descriptor = substr(descriptor, 2, 2)) %>% 
  filter(descriptor %in% 5:8) %>% 
  mutate(descriptor = recode_factor(descriptor, `5` = "DBHTWI", 
                                    `6` = "DBHBA", `7` = "DBHBAgenus",
                                    `8` = "DBHBAspecies")) %>% 
  mutate(species = as.numeric(species)) %>% 
  mutate(species = ifelse(complex == "Symphonia",
...
```{r predictionDBH, fig.cap="Predictions."}
mdata <- list(data %>% 
                filter(Complex == "Symphonia") %>% 
                mutate(DBHTWI = DBH*TWI, DBHBA = DBH*BA, 
                DBHBAgenus = DBH*BAgenus, DBHBAspecies = DBH*BAspecies),
              data %>% 
                filter(Complex == "Eschweilera clade Parvifolia") %>% 
                mutate(DBHTWI = DBH*TWI, DBHBA = DBH*BA, 
                DBHBAgenus = DBH*BAgenus, DBHBAspecies = DBH*BAspecies))
...
lapply(complexes, function(complex)
  lapply(1:length(unique(mdata[[complex]]$Species)), function(sp)
    cbind(dplyr::select(mdata[[complex]], DBHTWI, DBHBA, DBHBAgenus, DBHBAspecies),
          complex = complex,
          species = levels(as.factor(mdata[[complex]]$Species))[sp],
          theta = apply(as.matrix(fits[[complex]], 
                                  pars = paste0("theta[", 1:nrow(mdata[[complex]]), 
...
```{r nicheDBH, fig.cap="Niche (HabitatSuitability)."}
lapply(complexes, function(complex)
  lapply(1:length(unique(mdata[[complex]]$Species)), function(sp)
    cbind(mdata[[complex]],
          complex = complex,
  ...
  filter(species == Species) %>% 
  mutate(Species = paste0(substr(complex, 1, 1), ". ", Species)) %>% 
  dplyr::select(Species, DBH, theta, DBHTWI, DBHBA, DBHBAgenus, DBHBAspecies) %>% 
  reshape2::melt(id.vars = c("Species", "DBH", "theta", "DBHTWI")) %>% 
  ggplot(aes(DBHTWI, value, col = theta, size = DBH)) +
  geom_point() +
  facet_grid(variable~Species, scales = "free") +
  scale_color_gradient2("Habitat\nsuitability", low = scales::muted("blue"), mid = "white",
                        high = scales::muted("red"), midpoint = 1/7) +

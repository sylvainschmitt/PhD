...
```{r}
log_likelihood <- function(theta, y){exp(sum(y*log(theta)+(1-y)*log(1-theta)))}
...
```{r }
walk <- function(theta_old, y, sigma_explore = 0.1){
   theta_new <- rnorm(1, mean = theta_old, sd = sigma_explore)
   if (theta_new < 0) {
     theta_new <- 10^-6 
   }
   if (theta_new > 1) {
   ...
   ratio <- (likelihood(theta_new, y)*dbeta(theta_new, 1, 1))/(likelihood(theta_old, y)*dbeta(theta_old, 1, 1))
   if(runif(1) < ratio) {
     return(theta_new)
   } else {
     return(theta_old)
...
```{r, echo=F}
data.frame(dbh = 0:100, growth = sapply(0:100, function(dbh) growth(dbh))) %>% 
